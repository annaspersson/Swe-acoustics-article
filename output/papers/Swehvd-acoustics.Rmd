...
get_category_separability <- function(
  ...
  data %>%
    group_by(Talker, category) %>%
    mutate(
      category_separability_F1 = case_when(category == category_A ~ (F1 - data[data$category == category_B,]$F1_centroid)^2),
      category_separability_F2 = case_when(category == category_A ~ (F2 - data[data$category == category_B,]$F2_centroid)^2),
      category_separability_F3 = case_when(category == category_A ~ (F3 - data[data$category == category_B,]$F3_centroid)^2),
      overall_separability = (sum(n) * sqrt(category_separability_F1 + category_separability_F2)) / n)
}
