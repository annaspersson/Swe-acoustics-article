...
One generation has passed since @kuronen2000's description of the acoustics of Central Swedish. Since then, no study of the Central Swedish vowel space in its entirety has been conducted, to the best of my knowledge. The changes noted in @persson-jaeger2023 and @pelzer-boersma2019 seem to suggest that important events have taken place since then.<!--what events?--> A systematic review of the Central Swedish vowel space and its acoustics is therefore called for. The purpose of this study is to present an up-to-date acoustic description of the Central Swedish vowel space as represented by the talkers in the SwehVd database. The main spectral and temporal cues to vowel identity are reported in static analyses [following e.g., @engstrand1999; @fant1969], as well as dynamic analyses, given the well documented importance of formant trajectories on vowel production and perception [e.g., @assmann-katz2005; @hillenbrand-nearey1999; @kuronen2000; @nearey-assmann1986]. The acoustic characteristics of SwehVd is subsequently compared to vowel data from an unreleased database of 8 (female = 4) talkers of Central Swedish from the Greater Stockholm area recorded in 1999 as part of the SweDia2000 project [@bruce1999], in order to qualitatively and quantitatively assess the scope of potential vowel shifts.
...
d.previous.work <- tibble(
  ...
  "Speech materials" = c("3-5 repetitions of all 21 vowels, different phonological contexts for each vowel",
                         ...
                         "a subset of 19 vowels from the SweDia 2000 database (see Bruce et al., 1999 above)",
...
The acoustic analysis of SwehVd was compared against that of 8 reference talkers in the SweDia2000 materials (described below) in order to investigate possible vowel shifts over time. In this comparison, a distance measure was used in order to quantitatively assess whether the relative placement of the categories in the acoustic space had changed between datasets. For this purpose, the *orthogonal projection ratio* [henceforth, $op$; @stevens-schiel2019] was implemented. This metric, formalized in \@ref(eq:projection-ratio), compares the relative position of a category $V_{vowel}$ to two other anchor categories, $U_{anchor}$ and $W_{anchor}$. Typically, the anchor categories are selected based on their position in the space relative to the category of interest, $V_{vowel}$, that should be placed somewhere in between the anchors. The relative proximity of the vowel $V_{vowel}$ between the anchor categories is then measured. This is visualized in Figure XXX.

\begin{equation}\label{eq:projection-ratio}
...
d.SwehVd.forStudy.long.static %>%
  ...
  filter(Normalization.Type == "Nearey's uniform scaling (log)", Location %in% c(35, 50, 65), 
         #category %in% c("[ɪ]", "[yː]", "[ʏ]", "[ʉː]", "[ɵ]", "[eː]", "[ɛː]", "[æː]", "[øː]", "[ø]", "[œː]", "[œ]", "[ɑː]", "[a]", "[oː]", "[ɔ]")) %>%
         category %in% c("[ʉː]", "[oː]", "[ɪ]", "[eː]", "[æː]", "[ɑː]", "[ɵ]")) %>%
  # Get the geometric mean F1-F2 across the five time points
  ...
  gghighlight::gghighlight(category %in% c("[ʉː]", "[oː]", "[ɪ]"))
```
...
The two datasets were of unequal sample sizes: N = `r nrow(d.SwehVd %>% distinct(Talker, category, Token))` datapoints for SwehVd, and N = `r nrow(d.SweDia2000 %>% distinct(Talker, category, Token))` datapoints for SweDia2000. In order to address this imbalance, the SwehVd materials was downsampled to match the size of the SweDia2000 for the quantitative assessment of vowel shifts. This sampling-with-replacement procedure included 1000 bootstrapping rounds. At each round, a sample of 840 tokens from the SwehVd was extracted by first randomly selecting 8 talkers (4 female) from the database, and then randomly selecting 5 tokens per vowel from each talker, with replacement. The $op$ was calculated at each round, and finally averaged across the 1000 rounds for a comparison against the $op$ calculated for the SweDia2000 materials.
...
<!--is this needed-->SwehVd follows much of the previous studies on vowel production [e.g., @hillenbrand1995; @Peterson1952] and perception [e.g., @malinasky2020; @Peterson1952], and uses hVd words for recording, in order to minimise coarticulatory effects from the surrounding phonetic context. Using this context facilitates comparison to similar databases of other languages, but it nevertheless differs from the majority of previous studies on Swedish vowels, which have either not held the phonetic context constant across vowels [e.g., @bruce1999; @kuronen2000; @pelzer-boersma2019], or have investigated vowel production out of context [e.g., @eklund1997; @fant1969] or in different CVC contexts [e.g., *k*V*p* and *p*V*k* in @nordstrand2004]. One advantage of hVd is...<!--add here-->
...
For details on the recruitment, recording, pre-processing and annotation procedure in the SweDia2000 materials, see [@bruce1999].
...
In order to relate the vowel data in SwehVd to previous work and more directly assess potential shifts in the vowel space over time, a comparison to the reference talkers from the SweDia2000 corpus was conducted.
...
(ref:db-comparison) Vowel category movements in the Central Swedish vowel space from 1999 to 2023, in F1-F2. The arrow indicates the direction of movement over time, with base of arrow representing SweDia2000 (vowel label) and tip of arrow representing SwehVd. Vowel data is averaged across talkers in the two datasets.
