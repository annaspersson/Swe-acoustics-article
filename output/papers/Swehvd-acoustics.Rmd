...
The Central Swedish vowel space is traditionally characterized as having 9 phonemes that surface as long and short allophones due to a systematic quantity distinction (i.e., allophony by structural position) [@riad2014].^[Central Swedish is the regional standard variety of Swedish spoken in an area around and beyond Stockholm (eastern Svealand), including Mälardalssvenska, Sveamål, Uppsvenska, Mellansvenska [see e.g., @bruce2009; @elert1994; @riad2014].] Most of the long-short vowel pairs differ not only in quantity (duration) but also in quality (formant frequencies). Traditional descriptions of the entire vowel space rest on recordings made in the mid- or late $20^{th}$ century [e.g., @eklund1997; @engstrand1999; @fant1969], @kuronen2000 being the most recent acoustic mapping of the entire space, using recordings made in 1982 [@leinonen1982] [@leinonen2010's study constitutes an important exception, however, focused on dialectal variation]. The majority of recent studies on Central Swedish vowels have instead focused on subparts of the space, such as the long vowels for studies on diphthongization [e.g., @pelzer-boersma2019], or dialectal traits such the damped [`r linguisticsdown::cond_cmpl("ɨː")`] [@schotz2011], and mergers [@wenner2010], see Table \@ref(tab:previous-work). These studies all provide detailed mappings of different parts of the space, and contribute with important insights into ongoing processes of vowel shifts. However, given their focus on subsets of the space, the acoustic mapping of the modern-day Central Swedish vowel space *in its entirety* are largely lacking. 
In addition, most previous mappings of the vowel space, whether subparts or the entire space, differ in the materials used, in terms of phonological contexts used for recording, the size of the database (e.g., number of talkers, number of repetitions per vowel) and the demographics of talkers (e.g., male/female talkers, region of origin). This diversity restricts comparison across studies on Swedish, as well as cross-linguistically.

The current paper aims at providing a comprehensive up-to-date acoustic description of the Central Swedish vowel space, using a recently recorded database of Central Swedish h-VOWEL-d words, recorded by 44 male and female talkers of Central Swedish, the SwehVd database [@persson-jaeger2023]. The use of an hVd database minimizes coarticulatory effects from the surrounding phonetic context, and increase comparability to cross-linguistic studies and to future studies on Central Swedish [e.g., @hillenbrand1995; @Peterson1952]. However, it differs from the majority of previous studies on Swedish vowels, which have either not held the phonetic context constant across vowels, or have investigated vowel production out of context or in different CVC contexts (see Table \@ref(tab:previous-work). Figure \@ref(fig:swe-vowels) shows the vowel space as represented by the talkers in SwehVd. What was noted by @pelzer-boersma2019 in their study on diphthongization in the long vowels seem to be confirmed in this larger database as well---e.g., the high position of [`r linguisticsdown::cond_cmpl("eː")`], the centralized positions of [`r linguisticsdown::cond_cmpl("iː")`] and [`r linguisticsdown::cond_cmpl("yː")`], and low position of  [`r linguisticsdown::cond_cmpl("ɛː")`] compared to traditional descriptions. The centralized position of [`r linguisticsdown::cond_cmpl("iː")`] and [`r linguisticsdown::cond_cmpl("yː")`] is furthermore known to be a feature of the damped [`r linguisticsdown::cond_cmpl("ɨː")`] [e.g., @bjorsten-engstrand1999; @schotz2011], which has been reported among younger talkers of Stockholm Swedish [e.g., @riad2014; @schotz2011]. A second aim of this paper is to assess the scope and spread of these changes over the last generation. ...shifts, such as the damped [`r linguisticsdown::cond_cmpl("ɨː")`], involve a small number of participants [@schotz2011].

The current paper expands on previous work in two studies where the first study aims for a comprehensive acoustic description of the entire Central Swedish vowel space of 21 categories. The main spectral and temporal cues to vowel identity are reported in static analyses [following e.g., @engstrand1999; @fant1969], as well as dynamic analyses, given the well documented importance of formant trajectories on vowel production and perception [e.g., @assmann-katz2005; @hillenbrand-nearey1999; @kuronen2000; @nearey-assmann1986]. In contrast with previous work investigating the dynamics of Central Swedish vowels, the present study includes analyses of both long and short vowels. The qualitative analyses are supplemented with general additive mixed models to assess... GAMMs are particularly suitable for ... and are increasingly used in phonetic research.

The second study compares acoustic characteristics of SwehVd against vowel data from an unreleased database of 8 (female = 4) talkers of Central Swedish from the Greater Stockholm area recorded in 1999 as part of the SweDia dialect project [@bruce1999]. This comparison allows for an investigation of the scope of possible vowel shifts over time. Potential vowel shifts are assessed qualitatively by visualizations, and quantitatively using a distance metric, the orthogonal projection ratio [@stevens]. The use of a distance metric can presumably shed more light on the ...of changes over time.

The methodological approach adopted in the two studies expands on and complements previous work for a comprehensive picture of modern-day Central Swedish vowels. For instance, the use models for assessing the relative weight of different acoustic cues for vowel identity complements previous work on Central Swedish that have primarily used visual inspection and formant measurements (Table \@ref(tab:previous-work)). Furthermore, the present study is presumably the first to study the formant dynamics of both the long and short vowels.

The remainder of the paper is organized as follows. A background to the acoustics of Central Swedish vowels is provided by a review of previous work, the general methodology of the paper is then presented, including a description of the materials used. This is followed by the first study that presents study-specific methods together with results. Next, the second study's methods and results are presented, and finally, the results of both studies and their consequences for the Central Swedish vowel system is discussed. 
...
The qualitative acoustic descriptions and visualizations were supplemented with generalized additive mixed models (GAMMs) to model formant patterns dynamically, without having to rely on the simplifying assumption that each vowel can be reduced to a single F1-F2 point estimate. 
GAMMs are often used in phonetic research as they allow for complex models that can capture nonlinear relationships between predictors and response variables [e.g., @soskuthy2021]. Their use of smooth functions can provide a more nuanced representation of these relationships, especially when there are non-monotonic or dynamic trends in the data as is the case with formant trajectories.
Much like other mixed models, they also allow for the inclusion of random effects..reduces the risk of biasing models. <!--add here on advantages of hierarchical models-->The GAMMs used in this study included talker as random effect, which allowed the models to capture and account for individual differences, ensuring that the analyses were not overly influenced by idiosyncrasies of specific talkers in the database.<!--add here on gamm description-->
dynamic phonetic data, such as the present, not only have a temporal structure (formant measurements at different time-points), but also a hierarchical structure given that individual measurements are grouped into contours that in turn are grouped by words (hVd) and talkers.

The relative strength of each acoustic cue as a predictor for vowel category can also be assessed by performing statistical signifance tests on the GAMMs.


The \texttt{bam()} function from the \texttt{R} package \texttt{mgcv} was implemented to fit the GAMMs.

Of relevance for the present study, the GAMMs made the simplifying assumption of cue independence...
...
d.model.F3.long <- d.SwehVd.forStudy.long %>%
  ...
  mutate(across(c(Talker, category, Gender), ~ factor(.x)))
...
As is to be expected, vowels differing in quality are most separated in the F1-F2 panels. The F1-F3 and F3-F2 panels both display increased separation between the neighbouring [`r linguisticsdown::cond_cmpl("yː")`] - [`r linguisticsdown::cond_cmpl("ʉː")`] and [`r linguisticsdown::cond_cmpl("ɪ")`] - [`r linguisticsdown::cond_cmpl("ʏ")`], compared to when plotted along F1-F2, which points to the importance of F3 for rounded vowels. Interestingly, the almost complete overlap between [`r linguisticsdown::cond_cmpl("iː")`] and [`r linguisticsdown::cond_cmpl("yː")`] in F1-F2 space remains even when F3 is considered, which suggests that F3 might carry less importance as distinctive feature for [`r linguisticsdown::cond_cmpl("iː")`] - [`r linguisticsdown::cond_cmpl("yː")`] [c.f., @fant1959; @fant1969]. This was confirmed by running a generalized additive mixed model (GAMM) with F3 as dependent variable, gender and category and their interaction as independent variables, and time-points as smoothing functions. The intercept and slope of talker was included as random effect. Category was difference coded. The GAMM suggested that there was no main effect of category ($\bar\beta$ = , se = , $p < .82$) for the i-y distinction. For the short i-y, there was indeed a main effect of category ($p < .0001$), both results in line with the qualitative analysis.<!--first time of mentioning GAMMs, add info-->

<!--add fig of gamms here + analyses of other vowels-->
...
The aim of Study 2 was to investigate possible vowel shifts over time by comparing the acoustic analysis of SwehVd against that of 8 reference talkers in the SweDia materials. In this comparison, a distance measure was used in order to quantitatively assess whether the relative placement of the categories in the acoustic space had changed between datasets. For this purpose, the *orthogonal projection ratio* [henceforth, $op$; @stevens-schiel2019] was implemented.

## Methods
Visualizations in F1-F2 space...followed by op.<!--add here-->
...
```{r op, fig.width=base.width*2.5, fig.height=base.height*2.5, out.width='70%', fig.align='center', fig.cap="(ref:op)"}
d.SwehVd.forStudy.long.static %>%
  # Filter down to test folds
  filter(Normalization.Type == "Nearey's uniform scaling (log)", Location %in% c(35, 50, 65), 
