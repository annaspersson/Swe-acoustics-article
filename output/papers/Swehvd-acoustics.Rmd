...
The short vowels, Figure \@ref(fig:swe-vowels-by-quantity) right panel, present a slightly more compact space, however with increased category separability and less category overlap [c.f., @riad2014]. Of note, the high vowels [`r linguisticsdown::cond_cmpl("ɪ")`] and [`r linguisticsdown::cond_cmpl("ʏ")`] are more fronted than their long counterparts, which does not replicate previous studies on Central Swedish including both the long and short vowels [e.g., @fant1971; @kuronen2000].<!--check leinonen-->
...
To assess the magnitude of differences in $op$ between datasets, separate linear mixed-effects models (LMERs) for each of the categories investigated were fit using the \texttt{lmer()} function from the \texttt{R} package \texttt{lmerTest}. Example of model code: $op \sim Database + (1|Talker)$. Database was contrast coded with SweDia as reference category.
...
An estimate of the magnitude of the effect was obtained by fitting separate linear mixed-effects models (LMERs) for each of the categories, predicting the orthogonal projection ratio from database, with talkers as random effect. Summary tables of all 6 models are included in the SI, Section \@ref(sec:SI-LMERs). 

The LMERs confirmed the directionality of the vowel shifts, and found a main effect of Database on $op$ for all categories (all $ps < .019$), with the exception of [`r linguisticsdown::cond_cmpl("ʉː")`] ($\bar\beta = 0.11, SE = 0.071, p > .126$).^[LMERs including Word as random effect were also fit to the data, resulting in a main effect of database on [`r linguisticsdown::cond_cmpl("øː")`] only ($\hat\beta = .023, SE = .07, p < .005$). However, two of these models did not converge (for [`r linguisticsdown::cond_cmpl("œ")`] and [`r linguisticsdown::cond_cmpl("ʉː")`]), presumably due to the lack of datapoints, or because Word and Database correlated for one of the datasets (SwehVd) but was highly variable for the other dataset (SweDia). It is reasonable to assume that coarticulatory effects would be minimal given that the measurements are extracted from the steady-state of the vowel. Therefore, excluding Word as random effect likely had very little effect on the results.]
...
The objective of the present study was to provide an extensive acoustic analysis of contemporary Central Swedish vowels and to evaluate the extent of vowel shifts over the recent generation. The main findings from both studies are next discussed, alongside methodological considerations and future directions.
...
### Summary tables for LMERs fit to orthogonal projection ratio {#sec:SI-LMERs}
