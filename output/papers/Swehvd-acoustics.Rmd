...
# Abstract
The Swedish vowel space is a relatively densely populated space with 21 categories that differ in quality and quantity. Existing descriptions of the entire space rest on recordings made in the late 1990s or earlier, while recent work in general has focused on parts of the space [e.g., @engstrand1999; @fant1969; @kuronen2000; @pelzer-boersma2019; @riad2014; @schotz2011; @wenner2010]. The present paper reports two studies. The first study presents static and dynamic acoustic analyses of the entire vowel space using a recently released database of *h-VOWEL-d* words (SwehVd). The second study compares the acoustic characteristics of SwehVd against formant data from 8 talkers of the same variety recorded in 1999 [@eriksson2004], to investigate the scope and spread of previously reported shifts in the phonetic realization of some vowels [e.g., @bjorsten-engstrand1999; @pelzer-boersma2019; @riad2014]. This includes, for instance, the lowering and retraction of [`r linguisticsdown::cond_cmpl("ɛː")`] and the centralization of [`r linguisticsdown::cond_cmpl("iː")`] and [`r linguisticsdown::cond_cmpl("yː")`]. The results highlight the importance of F1, F2, duration and formant dynamics for Swedish vowel category distinction, and indicate possibly ongoing shifts in the high front part of the space.
...
Previous work on Swedish has analyzed vowel data using primarily Bark-transformed data [@fant1983; @kuronen2000; @schotz2011; @wenner2010], Mel-transformed data [@lindblom1963], Lobanov-normalized data [@gross2020], or unnormalized raw Hertz [@bjorsten-engstrand1999; @fant1969; @pelzer-boersma2019]. The choice of Nearey's uniform scaling in the present study was motivated by its previous use in socio-phonetic research to describe and compare languages and varieties [e.g., @barreda2021; @labov2001; @labov-boberg2005], and by its plausibility as perceptual model of how we come to achieve robust cross-talker perception, as it has provided a good fit against both production [e.g., @persson-jaeger2023; @syrdal1985] and perception data [e.g., @barreda2021; @persson2024]. <!--seems irrelevant, could go?-->Furthermore, its implementation closely resembles vocal tract length (VTL) normalization used in automatic speech recognition applications [e.g., @gouvea-stern1997].
Therefore, all analyses presented were applied on the normalized vowel data. Formant measurements in Hertz are reported in the SI, Section \@ref(sec:SI-raw).

## Datasets {#sec:datasets}
### The SwehVd database {#sec:swehVd}
The SwehVd database is a corpus of Swedish hVd word recordings, collected by Anna Persson and Maryann Tan (Stockholm University) in 2020-2024. An initial version of the corpus with 24 female talkers is described in @persson-jaeger2023. All recordings, annotations, and acoustic measurements are available on an OSF separate from the paper, at <https://osf.io/ruxnb/>. For this paper, an updated version of the database has been released, including an additional 20 male talkers. 
...
(ref:op) Illustration of orthogonal projection ratio. Examples of target categories $V_{vowel}$ (in cyan), and their respective anchor categories, $U_{anchor}$ and $W_{anchor}$ (in red). The $op$ measures the relative distance of $V_{vowel}$ to its anchors, as represented by the lines.
...
d.Swe.opr <- d.SwehVd.forStudy.long %>%
  # choose the anchor vowels
  filter(category %in% c("[ɪ]", "[ʏ]", "[yː]", "[ɵ]", "[eː]", "[ɛː]", "[æː]", "[œː]", "[œ]", "[ɑː]", "[oː]", "[øː]", "[ʉː]", "[ɔ]"), 
         Normalization.Type == "Nearey's uniform scaling (log)", timepoint %in% c(35, 50, 65)) %>%
  group_by(Database, Talker, Gender, category, Token, Quantity, Word) %>%
  summarise(across(c(F2, F1), ~mean(.x))) %>%
  # add the centroids %>%
  left_join(centroids.Swe, by = "Talker") 
