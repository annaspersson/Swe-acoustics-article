...
Existing descriptions of the entire space of 21 vowels rest on recordings made more than 25 years ago [reported in, e.g., @engstrand1999; @kuronen2000; @riad2014]. Two of the most recent studies are @leinonen2010 and @kuronen2000. The former is based on recordings obtained around 1999 of all but four of the short vowels.<!--change this--> It covers more than 1000 rural locations in Sweden, including reference talkers of Standard Swedish. The latter covers the entire vowel space but is based on recordings from 1981 [@leinonen1982]. More recent work over the last two decades has focused on parts of the phonological space instead, such as the long vowels for studies on diphthongization [@pelzer-boersma2019], two vowels for merger studies [e.g., [`r linguisticsdown::cond_cmpl("ɵ")`] - [`r linguisticsdown::cond_cmpl("œ")`] in @wenner2010], or a single vowel, such the damped [`r linguisticsdown::cond_cmpl("ɨː")`] [@schotz2011]. A summary of a selection of previous work on Central Swedish vowels is presented in Table \@ref(tab:SI-previous-work) in the Supplementary Information (SI). These studies all provide detailed mappings of different parts of the space, and contribute with important insights into ongoing processes and shifts. However, given their focus on subsets of the space, acoustic mappings of the modern-day Central Swedish vowel space *in its entirety* are largely lacking. Furthermore, it remains an open question how the use of small subsets might have affected the realizations of those vowels, as there is some evidence that productions of minimal pairs can lead to enhanced contrasts [@seyfarth; @buz; @lancia-winter]. In addition, most previous studies differ in the materials used, in terms of the size of the database (e.g., number of talkers, number of repetitions per vowel), the demographics of talkers (e.g., male/female talkers, region of origin), and phonological contexts used for recording. For instance, the majority of previous studies have either not held the phonetic context constant across vowels, or have investigated isolated vowel production out of context (Table \@ref(tab:SI-previous-work)). This diversity restricts comparison across studies on Swedish, as well as cross-linguistically.
...
The purpose of Study 1 is to .... <!--anticipate/state questions here-->which cues carry information about vowel quality once dynamic information is considered? Study 1 describes the spectral acoustics of Central Swedish in static and dynamic analyses of F0, F1, F2, F3, and duration. While fundamental frequency (F0) is not considered an important cue to vowel identity in itself, it is known to vary between languages, dialects and speech styles [e.g., @henton2005; @jacewicz-fox2018; @johnson2005; @mennen2012; @weirich2019] and is therefore also reported. Measurements of vowel duration are included to capture the systematic quantity distinction between long-short vowel pairs in Central Swedish.

The methodology employed in Study 1 is next presented, including a description of the materials used.

## Methods
### Materials {#sec:swehVd}
The materials used is a corpus of Swedish hVd word recordings, collected by Anna Persson and Maryann Tan (Stockholm University) in 2020-2024, the SwehVd. An initial version of the corpus with 24 female talkers is described in @persson-jaeger2023. For this paper, an updated release is presented, including 20 additional male talkers (targeted number of male talkers = 24). All recordings, annotations, and acoustic measurements are available on an OSF separate from the paper, at <https://osf.io/ruxnb/>. 
...
The short vowels (right panel), present a slightly more compact space, however with increased category separability [c.f., @riad2014]. Of note, the high vowels [`r linguisticsdown::cond_cmpl("ɪ")`] and [`r linguisticsdown::cond_cmpl("ʏ")`] are more fronted than their long counterparts, which does not replicate previous studies on Central Swedish [e.g., @fant1971; @kuronen2000].<!--check leinonen-->
...
p.merge <- d.SwehVd.forStudy.long.static %>%
  filter(Normalization.Type == "Nearey's uniform scaling (log)", Quantity == "long", Talker %in% c("SW_005", "SW_015"), category %in% c("[iː]", "[yː]", "[æː]", "[ɑː]", "[oː]", "[uː]")) %>%
  ggplot(
    aes(
      x = F2, 
      y = F1)) +
...
To gain more insight into the extent to which there are spectral and temporal differences between long and short vowels, the acoustics of categories within vowel pairs are evaluated. First, the overall differences in acoustics within vowel pairs are investigated. Then, we ask whether quantity and quality distinctions seem to be separate from each other.
...
# m.1 <- mgcv::bam(category ~ t2(F1, F2, F3, timepoint),
#                   family = multinomial(),
#                   data = d.model.paired)
...
The $op$, formalized in \@ref(eq:orthogonal-projection-ratio), compares the position of a single vowel $V_{vowel}$ from a talker, relative to the means of two anchor categories, $U_{anchor}$ and $W_{anchor}$, from the same talker. The anchor categories are selected based on their position in the space relatively to $V_{vowel}$, see Figure \@ref(fig:op). The $op$ for each token of $V_{vowel}$ from each talker relative to the by-talker anchors is calculated, and summarized across all tokens for each talker. This procedure is repeated for each of the $V_{vowel}$ and their respective anchors $U_{anchor}$ and $W_{anchor}$ for each talker in each dataset and then compared across datasets, in order to assess vowel shifts over time.
...
To assess the magnitude of differences in $op$ between datasets, separate linear mixed-effects models (LMMs) for each of the categories investigated were fit, predicting the orthogonal projection ratio from database, with talkers as random effect. Summary tables of all 6 models are included in the SI, Section \@ref(sec:SI-LMMs). The \texttt{lmer()} function from the \texttt{R} package \texttt{lme4} was used. Example of model code: $op \sim Database + (1|Talker)$. Database was treatment coded with SweDia as reference level.
The two databases employed different phonological context. One way to control for this difference would be to use random effects by word. However, this was not possible since this random effect would be highly collinear with the database, as SwehVd only used one word context. Indeed, most effects were no longer significant when word was included. Furthermore, it is reasonable to assume that coarticulatory effects would be minimal given that the measurements were extracted from the steady-state of the vowel. Therefore, excluding Word as random effect likely had very little effect on the results. 

The LMMs confirmed the directionality of the vowel shifts, and found a main effect of Database on $op$ for all categories (all $ps < .019$), with the exception of [`r linguisticsdown::cond_cmpl("ʉː")`] ($\bar\beta = 0.11, SE = 0.071, p > .126$).
