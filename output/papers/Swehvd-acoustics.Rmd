---
title             : "The spectral and temporal acoustics of Central Swedish vowels"
...
(ref:m-F2-long) Fitted smooths of GAMM for predicting F2 and 95% confidence intervals for vowels that overlap at different time-points of the vowel segment.
...
itsadug::plot_smooth(m.F2.long, view = "timepoint", plot_all = "category", col = c("#E7298A", "#A6D854", "#FFD92F", "#E5C494", "#A6CEE3", "#B2DF8A"), print.summary = FALSE, cond = list(category = c("[ɛː]", "[æː]", "[øː]", "[œː]", "[oː]", "[uː]")))
```
...
Next, differences in formant dynamics across the long and short vowel pairs are assessed.

##### Long-short vowel pair distinctions
The effect of F1, F2 and F3 on all vowel pairs was modeled in three separate GAMMs, with gender, quantity and vowel pair and their interaction as independent variables, and time-points as smoothing function. Random slopes and intercepts for talker was included. Category was backwards difference coded. Tables \@ref(tab:m-F1), \@ref(tab:m-F2) and \@ref(tab:m-F3) summarise the three GAMMs, and suggest main effect of category on both F1 ($\bar\beta$ = , se = , $p < .82$) and F2 ($\bar\beta$ = , se = , $p < .82$), with the exception of [`r linguisticsdown::cond_cmpl("æː")`] - [`r linguisticsdown::cond_cmpl("æ")`] ($p > .09$) predicting F1, thus confirming their overlap in height (Figure \@ref(fig:swe-vowels)).
...
# Rename contrast labels
d.model.paired$category <- factor(d.model.paired$category, labels = c("[ɪ]", "[iː]", "[ʏ]", "[yː]", "[ɵ]", "[ʉː]", "[ɛ]", "[eː]", "[ɛː]", "[æ]", "[æː]", "[ø]", "[øː]", "[œ]", "[œː]", "[a]", "[ɑː]", "[ɔ]", "[oː]", "[ʊ]", "[uː]"))

m.F1.paired <- mgcv::bam(F1 ~ category + Gender + Quantity + 
...
m.F2.paired <- mgcv::bam(F2 ~ Gender + Quantity * vowel_pair + 
                           s(timepoint, by = category, k = 5) + 
                           s(Talker, bs = "re") +
                           s(Talker, category, bs = "re"),
                         # + s(timepoint, Talker,  by = category, bs = "fs", k = 5, m=1)
...
m.F3.paired <- mgcv::bam(F3 ~ Gender + Quantity * vowel_pair + 
                           s(timepoint, by = category, k = 5) + 
                           s(Talker, bs = "re") +
                           s(Talker, category, bs = "re"),
                         # + s(timepoint, Talker,  by = category, bs = "fs", k = 5, m=1)
...
m.duration.paired <- mgcv::bam(Duration ~ Gender + Quantity * vowel_pair + 
                           s(timepoint, by = category, k = 5) + 
                           s(Talker, bs = "re") +
                           s(Talker, category, bs = "re"),
                         # + s(timepoint, Talker,  by = category, bs = "fs", k = 5, m=1)
...
and finally, on the F3 dynamics and its importance for the high front vowels...
...
# d.F3.highfront.long <- d.SwehVd.vowel_pairs %>%
...
# 
# # Difference code the vowel pair variable
# contrasts(d.F3.highfront.long$category) <- MASS::contr.sdif(3)
# 
# # Rename contrast labels
# d.F3.highfront.long$category <- factor(d.F3.highfront.long$category, labels = c("[iː]","[yː]", "[ʉː]"))
# 
# d.F3.highfront.short <- d.SwehVd.vowel_pairs %>%
#   filter(Normalization.Type == "Nearey's uniform scaling (log)", category %in% c("[ɪ]", "[ʏ]", "[ɵ]")) %>%
#   mutate(category = factor(category, levels = c("[ɪ]", "[ʏ]", "[ɵ]")))
# 
# # Difference code the vowel pair variable
# contrasts(d.F3.highfront.short$category) <- MASS::contr.sdif(3)
# 
# d.F3.highfront.short$category <- factor(d.F3.highfront.short$category, labels = c("[ɪ]", "[ʏ]", "[ɵ]"))

m.F3.highfront.long <- mgcv::bam(F3 ~ category + Gender + s(timepoint, by = category, k = 5) 
                                 + s(Talker, bs = "re")  
                                 + s(Talker, category, bs = "re"),
                                 # + s(timepoint, Talker,  by = category, bs = "fs", k = 5, m=1)
                                 data = d.model.long %>%
                                   filter(category %in% c("[iː]","[yː]", "[ʉː]")))

m.F2.highfront.long <- mgcv::bam(F2 ~ category + Gender + s(timepoint, by = category, k = 5) 
                                 + s(Talker, bs = "re")  
                                 + s(Talker, category, bs = "re"),
                                 # + s(timepoint, Talker,  by = category, bs = "fs", k = 5, m=1)
                                 data = d.model.long %>%
...
m.F1.highfront.long <- mgcv::bam(F1 ~ category + Gender + s(timepoint, by = category, k = 5) 
                                 + s(Talker, bs = "re")  
                                 + s(Talker, category, bs = "re"),
                                 # + s(timepoint, Talker,  by = category, bs = "fs", k = 5, m=1)
                                 data = d.model.long %>%
...
m.F3.highfront.short <- mgcv::bam(F3 ~ category + Gender + s(timepoint, by = category, k = 5) 
                                  + s(Talker, bs = "re")  
                                  + s(Talker, category, bs = "re"),
                                  # + s(timepoint, Talker,  by = category, bs = "fs", k = 5, m=1)
                                  data = d.model.short %>%
                                    filter(category %in% c("[ɪ]", "[ʏ]", "[ɵ]")))

m.F2.highfront.short <- mgcv::bam(F2 ~ category + Gender + s(timepoint, by = category, k = 5) 
                                  + s(Talker, bs = "re")  
                                  + s(Talker, category, bs = "re"),
                                  # + s(timepoint, Talker,  by = category, bs = "fs", k = 5, m=1)
                                  data = d.model.short %>%
...
m.F1.highfront.short <- mgcv::bam(F1 ~ category + Gender + s(timepoint, by = category, k = 5) 
                                  + s(Talker, bs = "re")  
                                  + s(Talker, category, bs = "re"),
                                  # + s(timepoint, Talker,  by = category, bs = "fs", k = 5, m=1)
                                  data = d.model.short %>%
...
itsadug::plot_smooth(m.F3.highfront.long, view = "timepoint", plot_all = "category", col = c("#66C2A5", "#FC8D62", "#8DA0CB"), print.summary = FALSE, cond = list(category = c("[iː]", "[yː]", "[ʉː]")))
```
...
itsadug::plot_smooth(m.F3.highfront.short, view = "timepoint", plot_all = "category", col = c("#1B9E77", "#D95F02", "#7570B3"), print.summary = FALSE, cond = list(category = c("[ɪ]", "[ʏ]", "[ɵ]")))
```
