...
header-includes:
 ...
 - \usepackage{setspace}\doublespacing #for double-spacing within columns and rows, this allows table 1 to fit within one page
 - \usepackage{subfig}
 - \usepackage{lscape}
 - \extrafloats{100} 
...
d.SwehVd.exclusions <-
  ...
  summarise(
    ...
    Transcribed_vowel_list = list(Transcribed_vowel),
    .groups = "drop") %>%
  unnest(cols = c(Transcribed_vowel_list)) %>%
  filter(Transcribed_vowel_list != "targeted") %>%
  group_by(Talker, Gender, category, N_tokens_per_category) %>%
  summarise(
    N_unidentifiable_vowels = n(),
    .groups = "drop") %>%
...
# # Write csv file with data used in study
# write.csv(
#   d.SwehVd.forStudy,
#   file = "../../data/phonetic vowel statistics/Swedish/Published databases/Persson-2023/outlier-corrected-vowel-data/Persson_2023_L1_vowels_wExlusions.csv",
...
p <- d.SwehVd.forStudy.long.static %>%
  # reformat category to print without brackets
  ...
  geom_label(
    ...
    mapping = aes(
      colour = category,
      label = category,
      fontface = ifelse(category %in% levels.vowel.IPA.swe.long.plot, 2, 1)),
    alpha = .8,
    size = 4,
    label.size = NA,
...
p.matrix.cues <-
  ...
  geom_point(
    ...
    mapping = aes(
      ...
      shape = Quantity),
    alpha = .8, size = 2) +
  scale_colour_manual(name = "category", values = colors.vowel.swe.plot, aesthetics = c("colour", "fill")) +
...
### Quantity versus quality in long and short vowel pairs
...
p.trajectories <- d.SwehVd.forStudy.long.dynamic %>%
  ...
  scale_color_manual(name = "category", values = colors.vowel.swe.plot) +
...
(ref:trajectory-ellipses) Vowel placement in F1-F2 space at each of the five time-points. Ellipses show bivariate Gaussian 95% confidence intervals of vowel means at each of the five time-points. Transparency indicates time-point, more transparent ellipses for earlier time-points. The vowel label is placed at vowel mid-point (50%). The arrow indicates the direction of the formant trajectory and ends at the final time-point, at 80% into the vowel.
...
```{r m-high-front-I-Y, message=FALSE, fig.width=base.width*2.5, fig.height=base.height*3, fig.align='center', fig.cap="(ref:m-high-front-I-Y)"}
par(mfrow=c(3,2))
...
```{r m-high-front-I-U-E, message=FALSE, fig.width=base.width*2.5, fig.height=base.height*3, fig.align='center', fig.cap="(ref:m-high-front-I-U-E)"}
par(mfrow=c(3,2))
...
```{r m-high-back, message=FALSE, fig.width=base.width*2.5, fig.height=base.height*3, fig.align='center', fig.cap="(ref:m-high-back)"}
par(mfrow=c(3,2))
...
```{r m-center, message=FALSE, fig.width=base.width*2.5, fig.height=base.height*3, fig.align='center', fig.cap="(ref:m-center)"}
par(mfrow=c(3,2))
...
```{r m-mid, message=FALSE, fig.width=base.width*2.5, fig.height=base.height*3, fig.align='center', fig.cap="(ref:m-mid)"}
par(mfrow=c(3,2))
...
The SwehVd dataset presented in this study can be found in an online repository (SwehVd: <https://osf.io/ruxnb/>). All analyses and visualization code for this paper can be found in a separate online repository (<https://osf.io/7uvj4/>).
...
# Acknowledgments {-}
I thank audiences at the 2023 ASA meeting, at the Ludwig-Maximilians-Universität Institute for Phonetics and Speech Processing lab meeting October 2023, and at FiNO 2024 for feedback on earlier presentations of this work. I am particularly grateful to Michele Gubian for advice and support in implementing GAMMs, T. Florian Jaeger for advice and support on code development, Maryann Tan for collaboration in preparation of the SwehVd database, and to Nathan Young for feedback on segmentation procedures, as well as for access to, and support for, the [SweFA software](https://github.com/mcgarrah/LG-FAVE) for the automatic annotation of Swedish vowels. Finally, I thank Niclas Abrahamsson, T. Florian Jaeger, Catrin Norrby, Rasmus Puggaard-Rode, Maryann Tan, Carla Wikse Barrow, and Marcin Wlodarczak for feedback on earlier versions of the manuscript.
