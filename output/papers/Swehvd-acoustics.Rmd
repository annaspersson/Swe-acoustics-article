...
The materials used for comparison is a database of N = `r n_distinct(d.SweDia2000.forStudy$Talker)` L1 talkers (N = `r nrow(d.SweDia2000.forStudy %>% filter(Gender == "female") %>% distinct(Talker))` female) of Central Swedish that were recorded as reference talkers of Standard Swedish for the SweDia dialect database [e.g., @eriksson2004]. All talkers were L1 talkers of Swedish, born and raised in the Greater Stockholm area or surroundings (provinces of Södermanland, Uppland), of 20-40 years of age (mean age = `r mean(d.SweDia2000.forStudy$Age, na.rm = T) %>% round(digits = 0)`; SD = `r sd(d.SweDia2000.forStudy$Age, na.rm = T) %>% round(digits = 2)`). The talkers were recorded reading a list of 41 words containing all 21 vowels of Central Swedish (for the complete wordlist used for recording, see Table \@ref(tab:word-list) in SI). Two of the words were bi-syllabic (*leta*, *flytta*), however, the target vowels extracted ([`r linguisticsdown::cond_cmpl("eː")`], [`r linguisticsdown::cond_cmpl("ʏ")`]) had primary stress and were thus included in this study. The rest of the words were all monosyllabic.
...
The objective of modelling formant dynamics is to investigate patterns across formant trajectories and assess the un/certainty by which we can predict acoustic cues from categorical or continuous predictors, such as word contexts and talkers. We would want to know to what extent vowels can be distinguished by certain acoustic cues, while accounting for by-talker variability and fluctuations in formants across the vowel segment. Modelling formant dynamics thus implies that the analysis has not only a temporal structure (formant measurements at different time-points), but also a hierarchical structure, as individual measurements are grouped into contours that in turn are grouped by words and talkers. 

Both the complex dependencies generated by this structure as well as the non-monotonic behavior of formants, cannot easily be captured by traditional parametric methods, such as linear regression [e.g., @soskuthy2021]. GAMMs allow for complex models that can accommodate nonlinear relationships between predictors and response variables, and their use of smooth functions can provide more nuanced representations of these relationships. Much like other mixed models, GAMMs allow for the inclusion of random effects, which permits the model to account for dependencies in the grouped data. Given that mixed models furthermore explicitly model the different sources of variability within and between groups, they can provide more precise estimates of parameters and reduce the risk of overfitting. GAMMs have previously been used in studies on vowels in different English varieties, e.g., on /u/-fronting in Derby English [@soskuthy2018], on the front vowel system of Southern American English [@renwick-stanley2020], but to the best of my knowledge, they have yet not be implemented in studies of Swedish vowels.
...
Figure \@ref(fig:trajectories) further demonstrates that some neighbouring categories either start at different locations but end up in the same, or start at similar locations but travel in different directions. For instance, [`r linguisticsdown::cond_cmpl("uː")`] - [`r linguisticsdown::cond_cmpl("oː")`] and [`r linguisticsdown::cond_cmpl("eː")`] - [`r linguisticsdown::cond_cmpl("iː")`] are fairly closely located at earlier time-points, but differ substantially towards the end of the vowel segment, while the [`r linguisticsdown::cond_cmpl("ɛː")`]-[`r linguisticsdown::cond_cmpl("æː")`], [`r linguisticsdown::cond_cmpl("øː")`]-[`r linguisticsdown::cond_cmpl("œː")`] and [`r linguisticsdown::cond_cmpl("ø")`]-[`r linguisticsdown::cond_cmpl("œ")`] pairs start at different locations but end up in approximately the same. Finally, the formant trajectories suggest that the empty spots identified in the vowel space under a static analysis (Figure \@ref(fig:swe-vowels)), may indeed be occupied when vowel dynamics is considered. This is especially true for [`r linguisticsdown::cond_cmpl("eː")`] that travels from the mid-high front to the mid center of the space as the signal unfolds---down to a position closer to its short counterpart, [`r linguisticsdown::cond_cmpl("ɛ")`]. Given the amount of overlap when static spectral cues are considered (Figure \@ref(fig:swe-vowels)), formant dynamics is likely highly informative for several of these distinctions.

#### Models of formant dynamics
The first set of GAMMs were modeled separately for each cue---F1, F2 and F3---and each set of long and short vowels, with gender, category and their interaction as independent variables, and time-points as smoothing function. Random slopes and intercepts for talker were included. Category was backwards difference coded with [`r linguisticsdown::cond_cmpl("iː")`] and [`r linguisticsdown::cond_cmpl("ɪ")`] as reference categories in each set, using the \texttt{contr.sdif()} function from the \texttt{R} package \texttt{MASS}. Example of model code: $F1 \sim category + Gender + s(timepoint, by = category, k = 5) + s(Talker, bs = "re") + s(Talker, category, bs = "re")$. 

Summary tables of all 6 models are included in the SI, Section \@ref(sec:SI-GAMMs). The GAMMs suggested a main effect of category on F1 for all long and short vowels in both datasets (all $ps > .0024$), except for [`r linguisticsdown::cond_cmpl("ʏ")`] ($\hat\beta = .012, se = .014, p > .37$). For both F2 and F3, there was a main effect of category for all long and short vowels (for F2, all $p > .037$; for F3, all $p > .046$), with the exception of [`r linguisticsdown::cond_cmpl("yː")`] for F3 ($\hat\beta = -.003, se = .011, p > .80$). The fact that a main effect of category on F3 was only found for the [`r linguisticsdown::cond_cmpl("ɪ")`] - [`r linguisticsdown::cond_cmpl("ʏ")`] and not the [`r linguisticsdown::cond_cmpl("iː")`] - [`r linguisticsdown::cond_cmpl("yː")`] distinction, suggests that F3 does not reliably distinguish between the long high front rounded-unrounded vowels, in line with the qualitative analysis. Figures \@ref(fig:m-F1-long), \@ref(fig:m-F1-short), \@ref(fig:m-F2-long), \@ref(fig:m-F2-short), \@ref(fig:m-F3-long), and \@ref(fig:m-F3-short) visualize the GAMMs. The clear diphthongization in [`r linguisticsdown::cond_cmpl("eː")`] - [`r linguisticsdown::cond_cmpl("oː")`] is evident in Figures \@ref(fig:m-F2-long) and \@ref(fig:m-F2-short), whereas Figures \@ref(fig:m-F3-short) and \@ref(fig:m-F3-long) highlight the importance of F3 for the [`r linguisticsdown::cond_cmpl("ɪ")`] - [`r linguisticsdown::cond_cmpl("ʏ")`], but not the [`r linguisticsdown::cond_cmpl("iː")`] - [`r linguisticsdown::cond_cmpl("yː")`] distinction, and finally, Figures \@ref(fig:m-F1-long) and \@ref(fig:m-F2-long) illustrate how some categories either overlap at the beginning or at the end of the vowel segment (e.g., [`r linguisticsdown::cond_cmpl("ɛː")`]-[`r linguisticsdown::cond_cmpl("æː")`], [`r linguisticsdown::cond_cmpl("øː")`]-[`r linguisticsdown::cond_cmpl("œː")`]).
...
The results thus seem to suggest that *all* long-short Central Swedish vowel pairs differ in spectral and temporal cues, including pairs for which previous research have found primarily duration dependencies [e.g., @behne-kirk1997; @haddingkoch1964; @kuronen2000].

### Results summary
The static and dynamic analyses suggest that all cues investigated in this study are of importance for Central Swedish vowel distinctions. A relatively indirect estimate of how much each cue contributed to vowel distinctions in the entire 21-category space was obtained by comparing the proportion of explained deviance in the GAMMs predicting F1, F2, F3 and duration from all vowel categories. These models confirmed the general pattern, suggesting that F1 (93.7% of explained deviance), F2 (96.2% of explained deviance) and duration (92% of explained deviance) can more easily predict all 21 categories than F3 (68.5% of explained deviance), as indicated by their relatively higher proportion of deviance explained. However, it is reasonable to assume that formant dynamics plays a central role for some category distinctions, for instance, the [`r linguisticsdown::cond_cmpl("oː")`] - [`r linguisticsdown::cond_cmpl("uː")`] and [`r linguisticsdown::cond_cmpl("ɛː")`] - [`r linguisticsdown::cond_cmpl("æː")`] distinctions. The [`r linguisticsdown::cond_cmpl("iː")`] - [`r linguisticsdown::cond_cmpl("y:")`] distinction presumably needs additional cues not analyzed here, given that neither formant dynamics nor F1, F2, F3 seem to reliably distinguish between the two.

The dynamic analyses furthermore highlighted that for most of the categories, a larger portion of the dynamics resides in the later part of the segment. Of note is that all short vowels also display formant movements, which taken together, confirms formant dynamics as auxiliary cue to vowel distinctions [e.g., @refs; for perception, e.g., @assmann-nearey].
...
(ref:op) Visualization of orthogonal projection ratio. Examples of target categories $V_{vowel}$ (in cyan), and their respective anchor categories, $U_{anchor}$ and $W_{anchor}$ (in red). The $op$ measures the relative distance of $V_{vowel}$ to its anchors, as represented by the line.
