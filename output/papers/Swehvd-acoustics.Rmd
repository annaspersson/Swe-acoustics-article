...
Dynamic phonetic data, such as the present, not only have a temporal structure (formant measurements at different time-points), but also a hierarchical structure given that individual measurements are grouped into contours that in turn are grouped by words (hVd) and talkers. This generates complex dependencies that cannot easily be captured by 
...
The relative strength of each acoustic cue as a predictor for vowel category can furthermore be assessed by performing statistical significance tests on the GAMMs.
...
itsadug::plot_smooth(m.F3, view = "Location", plot_all = "category", subset = "category" %in% c("[iː]", "[yː]", "[ʉː]")) +
  facet_wrap(~ category)
```
