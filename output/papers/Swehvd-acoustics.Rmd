...
Three general observations can be made from Figure \@ref(fig:cat-sep-F3). First, column A suggests that category separability is overall lower for some contrasts when only F1 and F2 were considered, e.g., the [`r linguisticsdown::cond_cmpl("iː")`] - [`r linguisticsdown::cond_cmpl("yː")`], and [`r linguisticsdown::cond_cmpl("ɪ")`] - [`r linguisticsdown::cond_cmpl("ʏ")`] contrasts, presumably indicating their overlap in F1-F2 space. Second, including F3 overall increases category separability, more so for some contrasts than others. The proportional increase in category separability relative to F1-F2 baseline (Figure \@ref(fig:cat-sep-F3)column B) is largest for the [`r linguisticsdown::cond_cmpl("iː")`] - [`r linguisticsdown::cond_cmpl("yː")`], [`r linguisticsdown::cond_cmpl("ɪ")`] - [`r linguisticsdown::cond_cmpl("ʏ")`], and [`r linguisticsdown::cond_cmpl("yː")`] - [`r linguisticsdown::cond_cmpl("ʉː")`] contrasts. How much separability increases by adding F3 to the [`r linguisticsdown::cond_cmpl("iː")`] - [`r linguisticsdown::cond_cmpl("yː")`] contrast varies quite substantially between talkers (as indicated by the large confidence intervals). Indeed, when assuming talker-independent representations, the relative increase in separability is less pronounced for this contrast (SI \@ref(sec:SI-separability-talkerIndep)). Third, the [`r linguisticsdown::cond_cmpl("yː")`] - [`r linguisticsdown::cond_cmpl("ʉː")`] contrast seems to benefit most from the inclusion of F3, resulting in an overall larger increase in separability for the outrounded vs. inrounded contrasts over the unrounded vs. outrounded contrasts.
...
```{r cat-sep-long-short, fig.width=base.width*4.5, fig.height=base.height*4, out.width='90%', fig.align='center', fig.cap="(ref:cat-sep-long-short)", message=FALSE}
...
Beginning with the static analysis, the results seem to suggest that the most important cues to vowel identity were F1, F2 and duration, which replicated previous work [e.g., @kuronen2000; @lindblom1963]. Including F3 increased separability for all rounding contrasts (with what appears as substantial between-talker variability for the [`r linguisticsdown::cond_cmpl("iː")`] - [`r linguisticsdown::cond_cmpl("yː")`] contrast), which suggests that F3 adds information for rounding vs. unrounded neighboring contrasts. The category separability index calculated for the long-short vowel pairs suggested that the F1-F2-Duration cue combination maximized separability between vowels in all pairs, with the exception of the [`r linguisticsdown::cond_cmpl("yː")`] - [`r linguisticsdown::cond_cmpl("ʏ")`] contrast, for which F1-F2-F3 generated the highest separability. Duration thus seems to contribute less to separating these two long-short vowels than F3, suggesting that the two vowel differ relatively more in F3 than in duration (given the way category separability is calculated here). The lower F3 values found in the short [`r linguisticsdown::cond_cmpl("ʏ")`] would suggest less lip-rounding in the long /`r linguisticsdown::cond_cmpl("y")`/ relative to its short counterparts, under the assumption of F3 being indicative of lip-rounding in the high-front vowels.
