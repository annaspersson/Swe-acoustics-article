...
```{r swe-vowels, fig.width=base.width*4.5, fig.height=base.height*2.5, fig.align='center', fig.pos="ht", fig.cap="(ref:swe-vowels)"}
...
```{r swe-vowels-by-quantity, fig.width=base.width*4, fig.height=base.height*2.5, fig.align='center', fig.pos="ht", fig.cap="(ref:swe-vowels-by-quantity)"}
...
The LMMs fit to the data (presented in Section \@ref(sec:method-static)) indicated that including F3 improved category separability for all contrasts (all $ps < .001$). This suggests that the subtle differences observed by visual inspection were nevertheless significant (Summary tables in SI \@ref(sec:SI-LMMs-study1)).^[The alpha level for statistical significance used throughout the paper is $p < .05$.]

```{r}
#Calculate the category separability based on Xie & Jaeger 2020
get_category_separability <- function(data, category_A, category_B, cues) {
  
  # Calculate the mean G1 and G2 for the reference category
  A_means <- data %>%
    filter(category == category_A) %>%
    summarise(across(all_of(cues), mean, .names = "mean_{col}"))
  
  # Calculate the mean G1 and G2 for the target category
  B_means <- data %>%
    filter(category == category_B) %>%
    summarise(across(all_of(cues), mean, .names = "mean_{col}"))
  
  # Extract means for the reference and target categories
  A_means_vec <- as.numeric(A_means)
  B_means_vec <- as.numeric(B_means)
  
  # Calculate distances from reference to target means
  A_to_B_distances <- data %>%
    filter(category == category_A) %>%
    mutate(distance = sqrt(rowSums((select(., all_of(cues)) - B_means_vec)^2))) %>%
    pull(distance)
  
  # Calculate distances from target to reference means
  B_to_A_distances <- data %>%
    filter(category == category_B) %>%
    mutate(distance = sqrt(rowSums((select(., all_of(cues)) - A_means_vec)^2))) %>%
    pull(distance)
  
  # Compute separability and standard error for both directions
  separability_A_to_B <- sum(A_to_B_distances) / length(A_to_B_distances)
  separability_B_to_A <- sum(B_to_A_distances) / length(B_to_A_distances)
  SE_A_to_B <- sd(A_to_B_distances) / sqrt(length(A_to_B_distances))
  SE_B_to_A <- sd(B_to_A_distances) / sqrt(length(B_to_A_distances))
    
  # Calculate the average separability and standard error
  separability <- mean(c(separability_A_to_B, separability_B_to_A))
  SE <- geometric.mean(c(SE_A_to_B, SE_B_to_A))
...
get_category_separability <- function(
  ...
  {
  data %>% 
    filter(Normalization.Type == "Nearey's uniform scaling (log)", category %in% c(category_A, category_B)) %>%
    left_join(data_means %>% 
                filter(category %in% c(category_A, category_B)),
              by = "Normalization.Type") %>%
     rename(category = category.x, mean_category = category.y) %>%
    mutate(
      distance_F1F2 = sqrt((F1 - F1_mean)^2 + (F2 - F2_mean)^2),
      distance_F0F1F2 = sqrt((F0 - F0_mean)^2 + (F1 - F1_mean)^2 + (F2 - F2_mean)^2),
      ...
      distance_F1F2F3duration = sqrt((F0 - F0_mean)^2 + (F1 - F1_mean)^2 + (F2 - F2_mean)^2 + (F3 - F3_mean)^2),
      distance_F0F1F2F3duration = sqrt((F0 - F0_mean)^2 + (F1 - F1_mean)^2 + (F2 - F2_mean)^2 + (F3 - F3_mean)^2 + (Duration - Duration_mean)^2),
      SE_F1F2 = sd(distance_F1F2) / sqrt(length(distance_F1F2)),
      SE_F0F1F2 = sd(distance_F0F1F2) / sqrt(length(distance_F0F1F2)),
      ...
      SE_F1F2F3duration = sd(distance_F1F2F3duration) / sqrt(length(distance_F1F2F3duration)),
      SE_F0F1F2F3duration = sd(distance_F0F1F2F3duration) / sqrt(length(distance_F0F1F2F3duration)),
      category_separabilityF1F2 = sum(distance_F1F2) / n(),
      category_separabilityF0F1F2 = sum(distance_F0F1F2) / n(),
      category_separabilityF1F2F3 = sum(distance_F1F2F3) / n(),
      category_separabilityF1F2duration = sum(distance_F1F2duration) / n(),
      category_separabilityF1F2F3duration = sum(distance_F1F2F3duration) / n(),
      category_separabilityF0F1F2F3duration = sum(distance_F0F1F2F3duration) / n())
  }
...
d.SwehVd.cat.separability <- rbind(
  ...
  summarise(
      ...
      F0F1F2_SE = geometric.mean(SE_F0F1F2),
      F1F2F3_SE = geometric.mean(SE_F1F2F3),
      F1F2duration_SE = geometric.mean(SE_F1F2duration),
      F1F2F3duration_SE = geometric.mean(SE_F1F2F3duration),
      F0F1F2F3duration_SE = geometric.mean(SE_F0F1F2F3duration),
      F1F2_separability = mean(category_separabilityF1F2),
      F0F1F2_separability = mean(category_separabilityF0F1F2),
      F1F2F3_separability = mean(category_separabilityF1F2F3),
      F1F2duration_separability = mean(category_separabilityF1F2duration),
      F1F2F3duration_separability = mean(category_separabilityF1F2F3duration),
      F0F1F2F3duration_separability = mean(category_separabilityF0F1F2F3duration))

  pivot_longer(
    cols = starts_with("F"),
    names_sep = "_",
    names_to = c("cue_dimension", "SE"),
  ...
  #mutate(cue_dimension = factor(cue_dimension, levels = c("F1F2", "F0F1F2", "F1F2F3", "F0F1F2F3duration")),
         category = factor(category, levels = levels.vowel.IPA.swe),
         Talker = factor(Talker),
         pairwise_separability = mean(separability))


# # Define the cues to be used
# cues_list <- list(c("F1", "F2"), 
#                   c("F1", "F2", "F3"), 
#                   c("F0", "F1", "F2"),
#                   c("F1", "F2", "Duration"),
#                   c("F1", "F2", "F3", "Duration"), 
#                   c("F0", "F1", "F2", "F3", "Duration"))
# 
# # Use purrr::map_dfr to iterate over the cues_list and apply the function
# d.SwehVd.cat.separability <- rbind(
#   tibble(
#     map_dfr(cues_list, function(cues) {
#       separability <- get_category_separability(d.SwehVd.for.cat.separability, category_A = "[iː]", category_B = "[yː]", cues)
#       data.frame(
#         combination = paste(cues, collapse = "-"),
#         avg_separability = separability$avg_separability,
#         avg_SE = separability$avg_SE
#       )
#     })) %>%
#     mutate(comparison = "iː - yː"),
#   tibble(
#     map_dfr(cues_list, function(cues) {
#       separability <- get_category_separability(d.SwehVd.for.cat.separability, category_A = "[eː]", category_B = "[yː]", cues)
#       data.frame(
#         combination = paste(cues, collapse = "-"),
#         avg_separability = separability$avg_separability,
#         avg_SE = separability$avg_SE
...
#   tibble(
#     map_dfr(cues_list, function(cues) {
#       separability <- get_category_separability(d.SwehVd.for.cat.separability, category_A = "[iː]", category_B = "[eː]", cues)
#       data.frame(
#         combination = paste(cues, collapse = "-"),
#         avg_separability = separability$avg_separability,
#         avg_SE = separability$avg_SE
...
#   tibble(
#     map_dfr(cues_list, function(cues) {
#       separability <- get_category_separability(d.SwehVd.for.cat.separability, category_A = "[ɪ]", category_B = "[ʏ]", cues)
#       data.frame(
#         combination = paste(cues, collapse = "-"),
#         avg_separability = separability$avg_separability,
#         avg_SE = separability$avg_SE
...
#   tibble(
#     map_dfr(cues_list, function(cues) {
#       separability <- get_category_separability(d.SwehVd.for.cat.separability, category_A = "[yː]", category_B = "[ʉː]", cues)
#       data.frame(
#         combination = paste(cues, collapse = "-"),
#         avg_separability = separability$avg_separability,
#         avg_SE = separability$avg_SE
...
#   tibble(
#     map_dfr(cues_list, function(cues) {
#       separability <- get_category_separability(d.SwehVd.for.cat.separability, category_A = "[ɛː]", category_B = "[æː]", cues)
#       data.frame(
#         combination = paste(cues, collapse = "-"),
#         avg_separability = separability$avg_separability,
#         avg_SE = separability$avg_SE
...
#   tibble(
#     map_dfr(cues_list, function(cues) {
#       separability <- get_category_separability(d.SwehVd.for.cat.separability, category_A = "[øː]", category_B = "[œː]", cues)
#       data.frame(
#         combination = paste(cues, collapse = "-"),
#         avg_separability = separability$avg_separability,
#         avg_SE = separability$avg_SE
...
#   tibble(
#     map_dfr(cues_list, function(cues) {
#       separability <- get_category_separability(d.SwehVd.for.cat.separability, category_A = "[ø]", category_B = "[œ]", cues)
#       data.frame(
#         combination = paste(cues, collapse = "-"),
#         avg_separability = separability$avg_separability,
#         avg_SE = separability$avg_SE
...
#   tibble(
#     map_dfr(cues_list, function(cues) {
#       separability <- get_category_separability(d.SwehVd.for.cat.separability, category_A = "[oː]", category_B = "[uː]", cues)
#       data.frame(
#         combination = paste(cues, collapse = "-"),
#         avg_separability = separability$avg_separability,
#         avg_SE = separability$avg_SE
...
#   tibble(
#     map_dfr(cues_list, function(cues) {
#       separability <- get_category_separability(d.SwehVd.for.cat.separability, category_A = "[ɔ]", category_B = "[ʊ]", cues)
#       data.frame(
#         combination = paste(cues, collapse = "-"),
#         avg_separability = separability$avg_separability,
#         avg_SE = separability$avg_SE
...
#       mutate(comparison = "ɔ - ʊ"))) %>%
#   mutate(
#     combination = factor(combination, levels = c("F1-F2", "F1-F2-F3", "F0-F1-F2", "F1-F2-Duration", "F1-F2-F3-Duration", "F0-F1-F2-F3-Duration")))
#     #mutate(cue_dimension = factor(cue_dimension, levels = c("F1F2", "F0F1F2", "F1F2F3", "F0F1F2F3duration")),
#     #category = factor(category, levels = levels.vowel.IPA.swe),
#     #Talker = factor(Talker))
```
...
```{r cat-sep-F3, fig.width=base.width*3, fig.height=base.height*1.75, out.width='90%', fig.align='center', fig.cap="(ref:cat-sep-F3)", message=FALSE}
# colors.comparison <- c("#FC8D62", "#FB9A99", "#1B9E77", "#8DA0CB", "#A6CEE3", "#33A02C")
# names(colors.comparison) <- c("iː - yː",  "eː - yː", "ɪ - ʏ", "yː - ʉː", "oː - uː", "ɔ - ʊ")

p.category.separability.unrounded <- d.SwehVd.cat.separability %>% 
  filter(comparison %in% c("iː - yː",  "eː - yː", "ɪ - ʏ")) %>%
  # mutate(
  #   baselineF3 = `avg_separability_F1-F2-F3` - `avg_separability_F1-F2`,
  #   baselineF0 = `avg_separability_F0-F1-F2` - `avg_separability_F1-F2`,
  #   baselineDuration = `avg_separability_F1-F2-Duration` - `avg_separability_F1-F2`) %>%
  mutate(
    comparison = factor(comparison, levels = c("iː - yː",  "eː - yː", "ɪ - ʏ")),
    rounding = "unrounded vs. outrounded") %>%
  pivot_longer(
    cols = -c(comparison, rounding),
    names_to = c("cues", "measure"),
    names_sep = "_") %>%
  pivot_wider(
    names_from = "measure",
    values_from = "value") %>%
...
p.category.separability.unrounded.overall <- d.SwehVd.cat.separability %>% 
     filter(comparison %in% c("iː - yː",  "eː - yː", "ɪ - ʏ")) %>%
     summarise(
       F1F2_overall = mean(F1F2_separability),
       F0F1F2_overall = mean(F0F1F2_separability),
       F1F2F3_overall = mean(F1F2F3_separability),
       F1F2duration_overall = mean(F1F2duration_separability),
       F1F2_SE = mean(F1F2_SE),
       F0F1F2_SE = mean(F0F1F2_SE),
       F1F2F3_SE = mean(F1F2F3_SE),
       F1F2duration_SE = mean(F1F2duration_SE)) %>%
     pivot_longer(
       cols = starts_with("F"),
       names_to = c("cues", "measure"),
       names_sep = "_") %>%
...
p.category.separability.inrounded <- p.category.separability.unrounded %+%
  ...
  #   baselineDuration = `avg_separability_F1-F2-Duration` - `avg_separability_F1-F2`) %>%
  mutate(
    comparison = factor(comparison, levels = c("yː - ʉː", "oː - uː", "ɔ - ʊ")),
...
p.category.separability.overall <- p.category.separability %+%
  (d.SwehVd.cat.separability %>% 
     filter(comparison %in% c("iː - yː",  "eː - yː", "ɪ - ʏ", "yː - ʉː", "oː - uː", "ɔ - ʊ")) %>%
     group_by(rounding) %>%
     mutate(
       F1F2_overall = mean(F1F2),
       F0F1F2_overall = mean(F0F1F2),
       F1F2F3_overall = mean(F1F2F3),
       F1F2duration_overall = mean(F1F2duration)))
#ggsave(file = "../../figures/p.png", width = 9, height = 5, dpi=1200)
```
```{r}
...
```{r duration-densities, fig.width= base.width * 3, fig.height = base.height * 1.75, fig.align='center', fig.pos='ht', out.width='95%', fig.cap="(ref:duration-densities)"}
...
In order to evaluate what cue combination would provide the largest separability between vowels in long-short contrasts, the category separability index was calculated for each pair and three different cue combinations: F1-F2, F1-F2-F3, or F0-F1-F2.^[For an approximation of the relative separability by cue, evaluations of F1-F3, F2-F3, F0-F1 and F0-F2 are included in the SI \@ref(sec:SI-separability).]
...
d.SwehVd.cat.separability.long.short <- rbind(
  ...
  summarise(
    ...
    F0F1F2F3duration = mean(category_separabilityF0F1F2F3duration, na.rm = T)) %>%
  pivot_longer(
    cols = starts_with("F"),
...
Beginning with the static analysis, the results seem to suggest that the most important cues to vowel identity were F1, F2 and duration, which replicated previous work [e.g., @kuronen2000; @lindblom1963]. For all roundings contrasts, F3 increased separability, which seem to suggest that F3 adds information. The category separability index calculated for the long-short vowel pairs suggested that categories in five of the quantity contrasts were just as separable in F1-F2 as F1-F2-F3 space. The contrasts for which statistically significant differences in separability index were found included two rounded pairs -- [`r linguisticsdown::cond_cmpl("yː")`] - [`r linguisticsdown::cond_cmpl("ʏ")`] and  [`r linguisticsdown::cond_cmpl("uː")`] - [`r linguisticsdown::cond_cmpl("ʊ")`], where significantly lower F3 values were found in the short vowels. Under the assumption of F3 being indicative of lip-rounding, this would suggest less lip-rounding in the long /`r linguisticsdown::cond_cmpl("y")`/ and /`r linguisticsdown::cond_cmpl("u")`/ relative to their short counterparts. 
...
GAMMs fit to the data contributed with further insights into which cues carry information about vowel quality once formant dynamics was considered. This was done for both neighboring contrasts hypothesized to depend on format dynamics, as well as for the quantity contrasts hypothesized to primarily differ in duration. For instance, GAMMs fitted to the long-short vowel pairs indicated that within all pairs, the two categories were significantly different in predicting the spectral cues in all cue evaluations, highlighting the informativity carried by formant dynamics for quantity distinctions. The static and dynamics analyses thus both suggest that quantity distinctions are not separate from quality distinctions in Central Swedish.

With regard to the neighboring contrasts hypothesized to rely on formant dynamics---the [`r linguisticsdown::cond_cmpl("iː")`] - [`r linguisticsdown::cond_cmpl("yː")`] - [`r linguisticsdown::cond_cmpl("ʉː")`] - [`r linguisticsdown::cond_cmpl("eː")`], [`r linguisticsdown::cond_cmpl("oː")`] - [`r linguisticsdown::cond_cmpl("uː")`], [`r linguisticsdown::cond_cmpl("ɛː")`] - [`r linguisticsdown::cond_cmpl("æː")`], and [`r linguisticsdown::cond_cmpl("øː")`]-[`r linguisticsdown::cond_cmpl("œː")`] contrasts, and their short counterparts---the results indicated significant differences for all comparisons and all cues with the exception of [`r linguisticsdown::cond_cmpl("iː")`] - [`r linguisticsdown::cond_cmpl("yː")`], [`r linguisticsdown::cond_cmpl("ɛː")`] - [`r linguisticsdown::cond_cmpl("æː")`], and [`r linguisticsdown::cond_cmpl("ɛ")`] - [`r linguisticsdown::cond_cmpl("æ")`] predicting F3, and [`r linguisticsdown::cond_cmpl("ɪ")`] - [`r linguisticsdown::cond_cmpl("ʏ")`], [`r linguisticsdown::cond_cmpl("øː")`]-[`r linguisticsdown::cond_cmpl("œː")`] predicting F1. This would seem to suggest that the movements in these vowels along these cues, are not contributing to vowel quality information. Presumably, the dynamics in remaining cues carries sufficient information for vowel quality. Alternatively, listeners might disambiguate the two categories using additional acoustic cues not investigated in this study, visual cues or linguistic information.

The vowel space as summarized in Table \@ref(tab:swehvd-vowels) suggests shifts in some vowels compared to previous characterizations of Central Swedish [c.f., @fant1969; @kuronen2000]. For instance, the lowering of [`r linguisticsdown::cond_cmpl("ɛː")`] which was anticipated by @riad2014, and confirmed in @leinonen2010, @gross2016, and @pelzer-boersma2019. Another important shift seem to concern the fronting of [`r linguisticsdown::cond_cmpl("ɪ")`] and [`r linguisticsdown::cond_cmpl("ʏ")`] and the centralization of [`r linguisticsdown::cond_cmpl("iː")`] and [`r linguisticsdown::cond_cmpl("yː")`], where [`r linguisticsdown::cond_cmpl("ɪ")`] and [`r linguisticsdown::cond_cmpl("ʏ")`] appear to maintain their positions as high front vowels, while [`r linguisticsdown::cond_cmpl("iː")`] and [`r linguisticsdown::cond_cmpl("yː")`] have centralized to the mid-center part of the space [see also, @pelzer-boersma2019].^[Unfortunately, @pelzer-boersma2019's study on diphthongization only included the long vowels, it is therefore difficult to know whether the fronting of the short vowels was as pronounced in 2019.] This conflicts with previous work on Swedish as well as other languages demonstrating that short vowels are generally less peripheral and more centralized than their long counterparts [e.g., @clopper2005; @hillenbrand1995]. This study's primary focus is mapping the acoustics of modern-day Central Swedish vowels, investigating the underlying reasons to these potential shifts is therefore left for future studies. However, awaiting future vowel change studies to corroborate these results, a speculative hypothesis would be that [`r linguisticsdown::cond_cmpl("iː")`] - [`r linguisticsdown::cond_cmpl("yː")`] are undergoing a merger, driven by relaxation of lip-rounding (as supported by the higher F3 values found for [`r linguisticsdown::cond_cmpl("yː")`]). An alternative hypothesis could be that both or either vowel is produced as damped versions, as previously found for talkers of Stockholm Swedish [e.g., @kotsinas1994; @schotz2011]. The presence of a damped [`r linguisticsdown::cond_cmpl("ɨː")`] would be supported by the lower F2 values, as the consonantal offglide in [`r linguisticsdown::cond_cmpl("ɨː")`] lowers F2 [@engstrand2000]. Impressionistic listening by the author did support the presence of a final consonantal glide, similar to [`r linguisticsdown::cond_cmpl("ɨː")`], among the majority of talkers in SwehVd, both male and female. The strength and scope of [`r linguisticsdown::cond_cmpl("ɨː")`] varied across talkers, from a relatively strong [`r linguisticsdown::cond_cmpl("ɨː")`] (17 talkers), to a weaker voiced fricative offglide buzzing [z] (12 talkers), or more of a consonant offglide element similar to [`r linguisticsdown::cond_cmpl("j")`] following [`r linguisticsdown::cond_cmpl("iː")`] (3 talkers). Interestingly, several of the talkers that did not produce any apparent final consonant glide or buzz (11 talkers), seemed to have overall less retracted [`r linguisticsdown::cond_cmpl("iː")`] and [`r linguisticsdown::cond_cmpl("yː")`], hence supporting the hypothesized link between centralization and consonantal offglide. These hypotheses could be further investigated in perceptual studies (see also Section \@ref(sec:methods-considerations)), or by using systematic listening by trained phoneticians, further validated through measures of inter-rater reliability [for a review, see @cucchiarini1995; @gross2020; @kuronen2000; @pelzer-boersma2019]. While evaluations by trained phoneticians can provide assessments of both auditory distinguishability between categories and the correspondence between assigned IPA label and auditory impression, training can differ across phoneticians and introduce biases [for reviews, see e.g., @heselwood-howard2008; @stemberger-bernhardt2020; @kerswill-wright1990].<!--add here--> Future studies should further assess the amount of cross-talker variability in these processes. This study suggest that even when keeping the background variables age and region of origin constant and normalizing for talker-specificity in physiology, there are still differences in the phonetic realization of some vowels that could be related to sociolinguistic differences. Further insights into these individual differences can be gained by studying the SwehVd materials on a talker-specific level when evaluating the scope and spread of vowel shifts.

While the static separability index suggested that F3-inclusion increased separability for all rounded contrasts, visualizations and models suggested that F3 did not contribute to the distinction between [`r linguisticsdown::cond_cmpl("iː")`] and [`r linguisticsdown::cond_cmpl("yː")`]. Under the assumption of F3 correlating with lip-rounding in high-front vowels, the lower F3 values in [`r linguisticsdown::cond_cmpl("ʏ")`] would seem to suggest more lip-rounding relative to [`r linguisticsdown::cond_cmpl("yː")`]. This finding conflicted with previous work on Central Swedish [e.g., @fant1969; @fujimura1967; @kuronen2000] and would seem to indicate that the  [`r linguisticsdown::cond_cmpl("iː")`] - [`r linguisticsdown::cond_cmpl("yː")`] contrast might primarily be supported by F1-F2 dynamics.

## Methodological considerations and future directions {#sec:methods-considerations}
Two sets of methodological considerations for the present work not mentioned elsewhere, deserve further discussion. The first concerns the measuring and evaluation of two of the acoustic cues, F0 and duration. Here, the average F0 across the vowel segment was reported, which necessarily excluded F0 from the dynamic analysis. However, pitch contours have been known to influence perceived duration and prominence of vowels [e.g., @gussenhoven-zhou2013]. One could therefore claim that a more comprehensive acoustic representation of vowels should have included pitch contours. Similarly, the temporal analysis of the effect of duration on long-short vowel pairs could be supplemented with measures of consonant ratios [e.g., @pelzer-boersma2019; @schaeffler2005]. These limitations can be addressed in future studies as the SwehVd database is publicly available in an online repository (<https://osf.io/ruxnb/>).

The second set of methodological considerations concerns methods used for assessing category distinguishability and how well certain cues can be predicted by neighbouring categories. Since the GAMMs were fit to each cue separately, they allowed for separate assessments of the relative weight of each cue, compared to the separability index where the by-cue contributions can only be evaluated indirectly. An inherent limitation in the separability index, as implemented in this paper, is the simplifying assumption that all dimensions within a cue combination carry equal weight. It is therefore not possible to assess whether the relation between the cues in each space is symmetrical or not, that is, in the F1-F2 space, we do not know whether F1 carries as much information as F2 for separability, and vice versa. In addition, given that the comparisons are pairwise, they are limited to explaining the relation between two vowels in a contrast. As such, they cannot inform us of the separability of a given vowel from other neighboring vowels, or the overall category separability in the entire space. This is, however, a limitation that the separability index shares with the GAMMs. Neither of these methods are able to assess the distinguishability or confusability of all vowels under different cue combinations in one analysis. Nor can they inform us of the *perceived* distinguishibility, even if it is reasonable to assume that reduced overlap between tokens of neighboring categories would increase intelligibility [e.g., @bradlow1995; @wright2004]. For this, one could fit other types of models, such as a multinomial logistic regression model predicting vowel category from cue combinations, or perceptual models assessing the predicted consequences for perception, predicting the intended category from production data. In separate work conducted in parallel with this study, I have pursued a similar approach, evaluating the predicted consequences of F3-inclusion for high-front vowel contrasts in Swedish using a perceptual model based on Bayesian inference, ideal observers [@persson-jaeger2024]. The results of including F3 qualitatively replicated the results presented here for the investigated vowels. While F3-inclusion overall improved the predicted recognition accuracy, it also increased confusability of the [`r linguisticsdown::cond_cmpl("iː")`] - [`r linguisticsdown::cond_cmpl("yː")`] contrast, suggesting the lack of informativity carried by F3. In comparison to other model-based approaches such as logistic regression and linear discriminant analysis, Bayesian ideal observers have the advantage of reducing the degrees of freedom in the fitting from production data to predicting perception, which substantially reduces the risk of over-fitting to the data [see discussion in e.g., @persson-jaeger2023]. This is made possible through committing to a strong linking hypothesis...[for a discussion on the effects of using a perceptual model over category separability indices, see also @persson-jaeger2023].<!--check inclusion here-->Besides accounting for the entire vowel space in one model, perceptual models can also aid in informing the design of perceptual studies that can shed more light on the consequences of the present results for the perception of Central Swedish vowels. There are many future avenues for perceptual studies... For instance, in a language with a systematic quantity distinction such as Swedish, the role of spectral cues in long-short vowel pair distinctions could be assessed by exposing listeners to synthesized versions where long and short vowel duration is crossed with the allophones' spectral information for any given phoneme. Furthermore, as the results seem to support claims of the hypothesized importance of formant dynamics for vowel distinctions, more insight into the effect of formant dynamics for vowel perception could be gained from having listeners categorize tokens extracted from different segments of the long vowels, e.g., the first three time-points vs. the three final time-points [c.f., @jenkins1994; @strange1989]. The design of such experiments can be informed by modeling the predicted perceptual consequences of different cue spaces, and of considering different vowel segments.
