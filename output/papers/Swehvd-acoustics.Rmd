...
```{r cat-sep-long-short, fig.width=base.width*4, fig.height=base.height*4.5, out.width='100%', fig.align='center', fig.cap="(ref:cat-sep-long-short)", message=FALSE}
...
Beginning with the static analysis, the results seem to suggest that the most important cues to vowel identity were F1, F2 and duration, which replicated previous work [e.g., @kuronen2000; @lindblom1963]. Including F3 increased separability for all rounding contrasts which suggests that F3 adds information for rounding vs. unrounded neighboring contrasts. The category separability index calculated for the long-short vowel pairs suggested that the F1-F2-Duration cue combination maximized separability between vowels in all pairs, with the exception of the [`r linguisticsdown::cond_cmpl("yː")`] - [`r linguisticsdown::cond_cmpl("ʏ")`] contrast, for which F1-F2-F3 generated the highest separability. Duration thus seems to contribute less to separating the vowels in this long-short contrast than F3. Under the assumption that F3 correlates with lip-rounding, this would suggest that the two vowels are relatively more separable in F3 than in duration (given the way category separability is calculated here).<!--changes that are larger than for other rounding contrasts, hence, supposedly goes beyond the expected reduction in F3 by producing a short vowel with less extreme articulatory movements than a long vowel. the f2 is higher for the short vowel so one would expect a higher F3 for the short over the long?-->
