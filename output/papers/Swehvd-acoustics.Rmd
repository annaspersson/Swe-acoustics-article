...
The Central Swedish vowel space is traditionally characterized as having 9 phonemes that surface as long and short allophones due to a systematic quantity distinction (i.e., allophony by structural position) [@riad2014].^[Central Swedish is the regional standard variety of Swedish spoken in an area around and beyond Stockholm (eastern Svealand), including Mälardalssvenska, Sveamål, Uppsvenska, Mellansvenska [see e.g., @bruce2009; @elert1994; @riad2014].] Most of the long-short vowel pairs differ not only in quantity (duration) but also in quality (formant frequencies). Traditional descriptions of the entire vowel space rest on recordings made in the mid- or late $20^{th}$ century [e.g., @eklund1997; @engstrand1999; @fant1969; @kuronen2000; @leinonen2010], as the majority of recent studies on Central Swedish vowels have focused on, e.g., diphthongization [e.g., @pelzer-boersma2019], dialectal traits such the damped [`r linguisticsdown::cond_cmpl("ɨː")`] [@schotz2011], and ongoing mergers [@wenner2010], see Table \@ref(tab:previous-work). These recent studies all provide detailed mappings of different parts of the space, and contribute with important insights into ongoing processes of vowel shifts. However, given their focus on subsets of the space, the characteristics of the modern-day Central Swedish vowel space *in its entirety* is less known. 
In addition, most previous mappings of the vowel space differ in the materials used, in terms of phonological contexts used for recording, the size of the database and the demographics of talkers. This diversity restricts comparison across studies on Swedish, as well as cross-linguistically. Assessments of scope and spread of ongoing changes in the space is furthermore restrained by the fact that many studies on vowel shifts, such as the damped [`r linguisticsdown::cond_cmpl("ɨː")`], involve a small number of participants [@schotz2011].

The current paper builds and expands on previous work in two studies that try to answer: what are the most important acoustic cues to vowel identity in modern-day Central Swedish, and to what extent has vowels shifted over the last generation? The first study aims for a comprehensive acoustic description of the entire Central Swedish vowel space of 21 categories, using a database of hVd words recorded by 44 male and female talkers of Central Swedish, the SwehVd database [@persson-jaeger2023], visualized in Figure \@ref(fig:swe-vowels). The use of an hVd database minimizes coarticulatory effects and increase comparability to cross-linguistic studies and to future studies on Central Swedish. The main spectral and temporal cues to vowel identity are reported in static analyses [following e.g., @engstrand1999; @fant1969], as well as dynamic analyses, given the well documented importance of formant trajectories on vowel production and perception [e.g., @assmann-katz2005; @hillenbrand-nearey1999; @kuronen2000; @nearey-assmann1986]. In contrast with previous work investigating the dynamics of Central Swedish vowels, the present study includes analyses of both long and short vowels. The qualitative analyses are supplemented with general additive mixed models to assess... GAMMs are particularly suitable for ... and are increasingly used in phonetic research. This complements previous work on Central Swedish that have primarily used visual inspection and formant measurements (Table \@ref(tab:previous-work)).

The second study compares acoustic characteristics of SwehVd against vowel data from an unreleased database of 8 (female = 4) talkers of Central Swedish from the Greater Stockholm area recorded in 1999 as part of the SweDia dialect project [@bruce1999]. This comparison allows for an investigation of the scope of possible vowel shifts over time. Potential vowel shifts are assessed qualitatively by visualizations, and quantitatively using a distance metric, an orthogonal projection ratio. The use of a distance metric can presumably shed more light on the ...of changes over time.

Overall, the methodological approach adopted in the two studies expands on previous work for a comprehensive picture of modern-day Central Swedish vowels. The remainder of the paper is organized as follows. A background to the acoustics of Central Swedish vowels is provided by a review of previous work, the general methodology of the paper is then presented, followed by the first study that presents study-specific methods together with results. Next, the second study's methods and results are presented, and finally, the results of both studies and their consequences for the Central Swedish vowel system is discussed. All analyses and visualization code for this study can be found in an online repository (<https://osf.io/>).<!--fill in-->
...
Using an hVd corpus for acoustic characterization of the vowel space both minimises coarticulatory effects from the surrounding phonetic context, and makes the description more cross-linguistically comparable [e.g., @hillenbrand1995; @Peterson1952]. However, it differs from the majority of previous studies on Swedish vowels, which have either not held the phonetic context constant across vowels, or have investigated vowel production out of context or in different CVC contexts (see Table \@ref(tab:previous-work).<!--move this?-->
...
```{r op, fig.width=base.width*2.5, fig.height=base.height*2, out.width='90%', fig.align='center', fig.cap="(ref:op)"}
d.SwehVd.forStudy.long.static %>%
  # Filter down to test folds
  filter(Normalization.Type == "Nearey's uniform scaling (log)", Location %in% c(35, 50, 65), 
...
The two datasets were of unequal sample sizes: N = `r nrow(d.SwehVd %>% distinct(Talker, category, Token))` datapoints for SwehVd, and N = `r nrow(d.SweDia2000 %>% distinct(Talker, category, Token))` datapoints for SweDia. In order to address this imbalance, the SwehVd materials was downsampled to match the size of the SweDia for the quantitative assessment of vowel shifts. This sampling-with-replacement procedure included 1000 bootstrapping rounds. At each round, a sample of 840 tokens from the SwehVd was extracted by first randomly selecting 8 talkers (4 female) from the database, and then randomly selecting 5 tokens per vowel from each talker, with replacement. The $op$ was calculated at each round, and finally averaged across the 1000 rounds for a comparison against the $op$ calculated for the SweDia materials.
...
To quantify these potential vowel shifts, the distances between the category locations in the two datasets were assessed with the $op$ [@stevens-schiel2019]. Table \@ref(tab:opr-table) lists the vowels tested along with their corresponding anchor points. These were all vowels for which the overall distance in F1 and F2 category means in SwehVd, compared to SweDia, $>= .75$ standard deviation from the category means in SweDia. Recall that given the incomparable sample sizes across the two datasets, a downsampling and bootstrap approach to the SwehVd materials was used to match the size of the SweDia reference talkers (\@ref(sec:investigating-change); the SI contain analyses that include calculations made without this bootstrap approach).
