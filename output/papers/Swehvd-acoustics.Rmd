...
A linear mixed-effects model (LMM) was fit to analyse the by-talker by-category separability for the pairs, with category, cue dimension and their interaction as fixed-effect predictors, and Talker as random effect.
...
d.SwehVd.for.cat.separability <- d.SwehVd.forStudy.long.static %>% 
  filter(Normalization.Type == "Nearey's uniform scaling (log)", category %in% c("[iː]", "[ɪ]", "[yː]", "[ʏ]")) %>%
  left_join(
    d.SwehVd.forStudy.long.static %>% 
      filter(Normalization.Type == "Nearey's uniform scaling (log)", category %in% c("[iː]", "[ɪ]", "[yː]", "[ʏ]")) %>%
      group_by(Talker, category) %>%
...
m.sep.YY <- lmerTest::lmer(value ~ cue_dimension + (1|Talker),
                      data = d.SwehVd.cat.separability %>% filter(category == "[yː]"))
