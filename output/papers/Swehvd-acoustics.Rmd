...
From previous research, we know the Central Swedish vowel space as a dense space of 21 categories. Static analyses have mapped out the space primarily along F1 and F2, but also concluded the importance of F3 for lip-rounding and duration for long-short vowel pair distinctions. Formant dynamics has furthermore been found to cue several long vowels, but little is known about its importance for the short vowels. In addition, phenomena such as the lowering of [`r linguisticsdown::cond_cmpl("ɛː")`], and the centralization of [`r linguisticsdown::cond_cmpl("iː")`] and [`r linguisticsdown::cond_cmpl("yː")`] have been reported in previous work [e.g., @leinonen2010; @schotz2011; @refs], however, they have often been studied using a limited number of talkers [but see @pelzer-boersma2019] and without comparing against their short counterparts or against the entire vowel space. The present study submits the entire vowel space to the same analyses, using a fairly large corpus of hVd words, for a comprehensive and hopefully more complete picture of modern-day Central Swedish vowels, while assessing the scope and spread of some previously reported changes in the space.
...
The objective of modelling formant dynamics is to investigate patterns in the data and assess the un/certainty by which we can predict acoustic cues from categorical or continuous predictors. We would thus want to know to what extent vowels can be distinguished by acoustic cues we believe them to differ by, while accounting for by-talker variability and fluctuations in formants across the vowel segment. Dynamic phonetic data, such as formant trajectories, tend to exhibit fluctuations and variability across the signal. A dynamic analysis of this type of data often involves comparisons across formant trajectories as a function of predictors, such as word contexts, talkers, etc. Thus, the analysis has not only have a temporal structure (formant measurements at different time-points), but also a hierarchical structure, as individual measurements are grouped into contours that in turn are grouped by words (hVd) and talkers. Both the complex dependencies generated by this structure as well as the non-monotonic behavior of formants, cannot easily be captured by traditional parametric methods, such as linear regression [e.g., @soskuthy2021]. GAMMs allow for complex models that can accomodate nonlinear relationships between predictors and response variables, and their use of smooth functions can provide more nuanced representations of these relationships. Much like other mixed models, GAMMs allow for the inclusion of random effects, which permits the model to account for dependencies in the grouped data. Given that mixed models furthermore explicitly model the different sources of variability within and between groups, they can provide more precise estimates of parameters and reduce the risk of overfitting. GAMMs have previously been used in studies on vowels in different English varieties, e.g., on /u/-fronting in Derby English [@soskuthy2018], on the front vowel system of Southern American English [@renwick-stanley2020], but to the best of my knowledge, they have yet not be implemented in studies of Swedish vowels.

In the present study, GAMMs were used for assessing the formant dynamics in subsets of the vowel space---contrasting some categories against each other, as well as the entire vowel space of 21 vowels. The GAMMs were fit separately for each of the formants, which necessarily meant committing to the simplifying assumption of cue independence. Previous work has shown that acoustic cues tend to co-vary [for a review, see @schertz-clare2019; @perception-book]. For vowels, this is the case for F1 and F2, as shown by the shape and orientation of ellipses in Figure \@ref(fig:swe-vowels-by-quantity).
...
d.SwehVd.vowel_pairs <- d.SwehVd.forStudy.long.dynamic %>%
  ...
  mutate(
    vowel_pair = case_when(category %in% c("[ɪ]", "[iː]") ~ "ɪ - iː",
                           ...
                           # Do three sets for the three-way contrast
                           category %in% c("[ɛ]", "[eː]") ~ "ɛ - eː",
                           category %in% c("[ɛ]", "[ɛː]") ~ "ɛ - ɛː",
...
The formant dynamics of the vowels were modeled separately for each cue and each set of long and short vowels...<!--add here to model explanation-->
...
m.F1.paired <- mgcv::bam(F1 ~ Gender + Quantity * vowel_pair + 
...
m.Ö <- lmerTest::lmer(orth_proj ~ Database + (1|Word) + (1|Talker), 
                      data = d.orth %>%
                        ...
                        mutate(across(c(Talker, Database), ~ factor(.x))))
m.ÖÖ <- lmerTest::lmer(orth_proj ~ Database + (1|Word) + (1|Talker), 
                       data = d.orth %>%
...
The objective of the present study was to provide an extensive analysis of the acoustic cues to vowel identity in contemporary Central Swedish and to evaluate the extent of vowel shifts over the recent generation. The main findings results from both studies are next discussed, alongside methodological considerations and potential avenues for future research.
...
# Conclusions
The present study has reported on the acoustic properties of Central Swedish vowels and how they have changed over the last generation. The spectral and temporal cues investigated all seem to contribute to distinguishing between the 21 vowels in the Central Swedish vowel space. More insight into formant dynamics within and between quantities have been gained by the dynamic analysis presented, which is of value for cross-linguistic research. 
...
p.talkers <- d.SwehVd.forStudy.long.static %>%
  filter(Normalization.Type == "Nearey's uniform scaling (log)", Gender == "female") %>%
  ggplot(
    aes(
      x = F2,
      y = F1)) +
...
p.talkers %+%
(d.SwehVd.forStudy.long.static %>%
  filter(Normalization.Type == "Nearey's uniform scaling (log)", Gender == "male"))
```
