...
(ref:swe-vowels) The SwehVd vowel data in unnormalized Hertz and Nearey's uniform scaling space, along F1 and F2. Points show recordings of each of the 21 Central Swedish vowels by 44 (24 female) L1 talkers in the database, at vowel midpoint. Vowel labels are placed at the category mean across talkers. Long vowels are boldfaced. Vowels that mismatched intended label are excluded (`r signif((d.SwehVd %>% filter(Transcribed_vowel != "targeted") %>% nrow() / d.SwehVd %>% nrow()) * 100)`% of all recordings).
...
To sum up, 
This is what we know, this is what we don't know, and this is what I'll do and next are the methods...
...
The general methods described below are shared by both studies, while study-specific methodology is presented under each study.
...
The dynamics of these vowels will be further investigated in Section \@ref(sec:swehVd-dynamics).
...
The spectral differences between the long-short vowel pairs were also investigated in dynamic analyses, see Section XXX below.

### Dynamic spectral analysis {#sec:swehVd-dynamics}
The dynamic analysis of the spectral acoustics of the long and short vowel begins with visualizations of the empirical data in formant trajectory plots. The spectral cues are then modeled with GAMMs... First, we investigate the movements in all 21 categories, and then zoom in on some category distinctions for which certain cues are hypothesized to be of importance. Finally, the systematicity in spectral differences between the long and short vowel pairs is analysed...To supplement the empirical analysis in the previous section, GAMMs were fit to... 
...
itsadug::plot_smooth(m.F3, view = "timepoint", plot_all = "category", col = c("#66C2A5", "#FC8D62", "#8DA0CB"), print.summary = FALSE, cond = list(category = c("[iː]", "[yː]", "[ʉː]")))
```
...
The dynamic analyses furthermore highlighted that for most of the categories, more substantial dynamics are displayed in the later part of the segment. Of note is also that all short vowels also display formant dynamics...most likely auxiliary cues to vowel categorization.
...
ADD outline paragraph + add subsection headers structured as the study as a whole - study 1 and study 2...
THIS SHOULD ALSO BE DISCUSSED:
- duration is not sufficient to distinguish between the long-short vowel pairs.
- the type of dynamic effect
...
<!--add suggestions for future research-->suggest perceptual studies + the fact that there were substantial individual differences, the repo is available. BUT mention prior to this section, the presumable perceptual consequences of the later part of the vowel - where much of the dynamic is going on.
