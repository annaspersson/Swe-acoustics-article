...
Existing descriptions of the entire space of 21 vowels rest on recordings made more than 25 years ago [reported in, e.g., @engstrand1999; @kuronen2000; @leinonen2010; @riad2014]. Two of the most recent studies are @leinonen2010 and @kuronen2000. The former is based on recordings obtained around 1999 of all vowels, although four of the short vowels were omitted from analysis. It covers more than 1000 rural locations in Sweden, including reference talkers of Standard Swedish. The latter covers the entire vowel space but is based on recordings from 1981 [@leinonen1982]. More recent work over the last two decades has focused on parts of the phonological space, such as the long vowels for studies on diphthongization [@pelzer-boersma2019], two vowels for merger studies [e.g., [`r linguisticsdown::cond_cmpl("ɵ")`] - [`r linguisticsdown::cond_cmpl("œ")`] in @wenner2010], or a single vowel, such the damped [`r linguisticsdown::cond_cmpl("ɨː")`] [@schotz2011], see Table \@ref(tab:previous-work). These studies all provide detailed mappings of different parts of the space, and contribute with important insights into ongoing processes and shifts. However, given their focus on subsets of the space, acoustic mappings of the modern-day Central Swedish vowel space *in its entirety* are largely lacking. Mapping the entire space at one go could generate a more accurate representation of the system. It remains an open question how representative subsets are for the vowel space as a whole, as there is some evidence that productions of minimal pairs can lead to enhanced contrasts that might thus have affected the realizations of those vowels [e.g., @seyfarth2016 for non-vowels; @lancia-winter]. <!--change this--> In addition, most previous studies differ in the materials used, in terms of the size of the database (e.g., number of talkers, number of repetitions per vowel), the demographics of talkers (e.g., male/female talkers, region of origin), and phonological contexts used for recording. For instance, the majority of previous studies have either not held the phonetic context constant across vowels, or have investigated isolated vowel production out of context or in different CVC contexts (Table \@ref(tab:previous-work)). This diversity restricts comparison across studies on Swedish, as well as cross-linguistically.
...
The materials used in Study 1 is a corpus of Swedish hVd word recordings, collected by Anna Persson and Maryann Tan (Stockholm University) in 2020-2024, the SwehVd. An initial version of the corpus with 24 female talkers is described in @persson-jaeger2023. For this paper, an updated release is presented, including 20 additional male talkers (targeted number of male talkers = 24). All recordings, annotations, and acoustic measurements are available on an OSF, at <https://osf.io/ruxnb/>. 
...
The static analysis of SwehVd presents formant measurements at the steady state of the vowel, by averaging across the three mid-points.^[The choice of time-point for extracting formants, or whether to average across several time-points, affects the acoustic characterizations given how formants move across the vowel segment. The SI presents evaluations of the effect of different measurement points. For increased comparability across studies in this paper, the steady state of the vowel was selected for both studies.] It investigates where in the acoustic space categories fall, and evaluates the relative contribution of F0, F1, F2, F3 and duration to the distinction of vowels. This is done using visualizations and category separability...<!--add here on separability measure--> To obtain a measure of the magnitude of effect, regression analyses were conducted.

#### Dynamic acoustic analysis
The dynamic analysis reports formant measurements at all five time-points in two main sections that overall aim to assess the importance of formant dynamics for vowel distinctions. In the first section, formant trajectory plots were used to assess the scope and direction of movements and to what extent vowel seem to diphthongize or merely display formant movements. The trajectory plots were also used to evaluate whether category overlap is reduced once trajectories are considered. This analysis assessed previously reported results on long vowels [@refs], and explored trajectories in the short vowels.
...
The hypotheses investigated in the static analysis were explored under the assumption of formant dynamics. Therefore, GAMMs were fit to all 21 categories, as well as to two categories within a vowel pair separately, to assess to what extent long and short vowels in a pair are contrasted.
The \texttt{bam()} function from the \texttt{R} package \texttt{mgcv} was used to fit GAMMs. Example of model code: $F1 \sim category + Gender + s(timepoint, by = category, k = 5) + s(Talker, bs = "re") + s(Talker, category, bs = "re")$. The first set of GAMMs were fit to the long and short vowels separately. Here, vowel was backwards difference coded with [`r linguisticsdown::cond_cmpl("iː")`] and [`r linguisticsdown::cond_cmpl("ɪ")`] as reference categories in each set, using the \texttt{contr.sdif()} function from the \texttt{R} package \texttt{MASS}. For the second set, evaluating differences between categories within vowel pairs, vowel was backwards difference coded with [`r linguisticsdown::cond_cmpl("iː")`] as reference vowel. The vowel variable was ordered alternating long and short vowels by the pair, in order for the short vowel to be compared against its preceding long counterpart. 
...
The static analysis begins with an evaluation of where in the F1-F2 space Central Swedish vowels live. Next, the relative contribution of additional cues beyond F1 and F2 is assessed, as well as the extent to which long-short vowel pairs are qualitatively different.
...
In order to quantitatively asses whether the distinction between the neighboring [`r linguisticsdown::cond_cmpl("iː")`] and [`r linguisticsdown::cond_cmpl("yː")`] increased when F3 was added, the category separability of these two vowels was calculated based on F1 and F2, and subsequently compared against the separability calculated on F1, F2 and F3. A linear mixed-effects model (LMM) was fit to analyse the by-talker by-category separability for the [`r linguisticsdown::cond_cmpl("iː")`] - [`r linguisticsdown::cond_cmpl("yː")`] pair, with category, cue dimension and their interaction as fixed-effect predictors, and Talker as random effect. Cue dimension was sum-coded (F1-F2 = 1, F1-F3 = -1).

Q1: how does separability change when F3 is added? Q2: does F3 predict rounding? estimate the consequence by calculating category separability.
...
d.SwehVd.for.cat.separability.all <- rbind(
  ...
  summarise(
    categorySeparability_F1F2 = mean(separability_F1F2, na.rm = T),
    categorySeparability_F1F3 = mean(separability_F1F2F3, na.rm = T)) %>%
  pivot_longer(
    cols = starts_with("categorySeparability"),
    names_to = "cue_dimension",
    values_to = "value") %>%
  mutate(cue_dimension = as.factor(cue_dimension))

contrasts(d.SwehVd.for.cat.separability.all$cue_dimension) <- contr.sum(2)

m.sep <- lmerTest::lmer(value ~ cue_dimension + (1|Talker),
                      data = d.SwehVd.for.cat.separability.all %>% filter(category == "[yː]"))

summary(m.sep)
```

#### Quantity vs. quality in long and short vowel pairs
To gain more insight into the extent to which there are spectral and temporal differences between long and short vowels, the acoustics of categories within vowel pairs are evaluated. This can allow for an assessment of whether quantity and quality distinctions seem to be separate from each other.
...
To sum up, the results of the static analysis suggests that F1 and F2 are the most important cues to vowel distinctions. While including F3 does lead to increased category separability for XX and XX, this does not seem to be the case for i-y. In addition, long-short vowel pairs display considerable spectral differences, suggesting that quantity and quality distinctions are not separable. However, we know that formants are not static but rather fluctuate across the signal. A more comprehensive mapping of the acoustics should thus aim to assess the relative contribution of each cue when dynamics is considered. Aiming to address one of the cue questions in formant dynamics, the next section investigates how much does dynamic information add to the relative weight of cues. Given that dynamics should hypothetically benefit perception, one could assume that category distinctions would increase once dynamics is included.

### Dynamic spectral analysis {#sec:swehVd-dynamics}
The dynamic analysis of the spectral acoustics begins with visualizations of the empirical data in formant trajectory plots. The aim is to assess...<!--repeat from above--add here--> and evaluate claims of diphthongization in previous work, as well as the extent to which short vowels also display formant movements. Beyond at the highest level characterize the trajectory movements across the segment, dynamics can tell us something about the classification of vowels and what part of the system undergo diphthongization. And second, do these dynamic movements add to the discriminability of Central Swedish vowels.

The spectral cues are then modeled with GAMMs, first focusing on the movements in all 21 categories, then on spectral differences between the long and short vowel pairs, in order to assess to what extent dynamic information adds to the relative contribution of each of the cues to vowel distinctions. I thus ask what cues are primarily affected when formant movements are considered?
...
In terms of distinguishing between what appears to be diphthongization and merely formant movement, Figure XXX suggests substantial diphthongization in primarily [`r linguisticsdown::cond_cmpl("eː")`] and [`r linguisticsdown::cond_cmpl("oː")`], which replicates previous work [e.g., @eklund1997; @elert2000; @pelzer-boersma2019]. The previously reported diphthongization in [`r linguisticsdown::cond_cmpl("øː")`] does not seem to be particularly pronounced in this material...<!--add here-->
...
<!--add here cat sep for all tps??-->Estimate whether category separability increases once dynamic information is considered, by calculating a separability index that takes the time-steps into account. Do some vowels benefit from formant movements disproportionally much? Treat each time-point as an independent Gaussian distribution, calculate at each time-point separate indices over the cues and then sum them up for the 5 tps.
