...
The same process was applied to investigate to what extent long-short vowel pairs differ in spectral and temporal cues, by assessing what combination of cues could provide the largest separability between the two vowels in each pair. For this evaluation of quantity contrasts, the category separability index was calculated for each pair and three different cue combinations: F1-F2, F1-F2-F3, or F1-F2-duration. The models were the same as the previous sets, however, cue combination was treatment-coded with F1-F2 as reference category, thus comparing each cue combination against the F1-F2 combination.
...
Three general observations can be made from Figure \@ref(fig:cat-sep-F3). First, column A suggests that category separability was overall lower for some contrasts when only F1 and F2 were considered, e.g., the [`r linguisticsdown::cond_cmpl("iː")`] - [`r linguisticsdown::cond_cmpl("yː")`], and [`r linguisticsdown::cond_cmpl("ɪ")`] - [`r linguisticsdown::cond_cmpl("ʏ")`] contrasts, presumably indicating their overlap in F1-F2 space. Second, including F3 overall increased category separability, more so for some contrasts than others. The increase in category separability relative to F1-F2 baseline (Figure \@ref(fig:cat-sep-F3)column B) was largest for the [`r linguisticsdown::cond_cmpl("iː")`] - [`r linguisticsdown::cond_cmpl("yː")`], [`r linguisticsdown::cond_cmpl("ɪ")`] - [`r linguisticsdown::cond_cmpl("ʏ")`], and [`r linguisticsdown::cond_cmpl("yː")`] - [`r linguisticsdown::cond_cmpl("ʉː")`] contrasts. Third, the [`r linguisticsdown::cond_cmpl("yː")`] - [`r linguisticsdown::cond_cmpl("ʉː")`] contrast seems to benefit most from the inclusion of F3, resulting in an overall larger increase in separability for the outrounded vs. inrounded contrasts over the unrounded vs. outrounded contrasts.

The LMMs fit to the data (presented in Section \@ref(sec:method-static)) indicated that including F3 improved category separability for all contrasts (all $ps < .004$). This suggests that the subtle differences observed by visual inspection for the [`r linguisticsdown::cond_cmpl("eː")`] - [`r linguisticsdown::cond_cmpl("yː")`], [`r linguisticsdown::cond_cmpl("oː")`] - [`r linguisticsdown::cond_cmpl("uː")`] and [`r linguisticsdown::cond_cmpl("ɔ")`] - [`r linguisticsdown::cond_cmpl("ʊ")`] contrasts were nevertheless significant (Summary tables in SI \@ref(sec:SI-LMMs-study1)).^[The alpha level for statistical significance used throughout the paper is $p < .05$.]
...
(p.category.separability.inrounded + facet_wrap(~rounding) + p.category.separability.diff.inrounded + facet_wrap(~rounding)) +
...
p.category.separability.diff.high <- p.category.separability.diff.unrounded %+%
  (d.SwehVd.cat.separability.comparison.longshort %>%
     ...
     pivot_longer(
       cols = starts_with("separabilityDifference"),
       names_to = "cue_difference") %>%
     rbind(d.SwehVd.cat.separability.comparison.longshort %>%
             select(comparison, height, separabilityDifference_F3, separabilityDifference_Duration) %>%
             filter(height == "high vowels") %>%
             group_by(comparison, height) %>%
             summarise(
               separabilityDifference_F3 = mean(separabilityDifference_F3),
               separabilityDifference_Duration = mean(separabilityDifference_Duration)) %>%
             pivot_longer(
               cols = starts_with("separabilityDifference"),
               names_to = "cue_difference") %>%
             mutate(comparison = "overall")) %>%
...
(p.category.separability.lowermid + facet_wrap(~height) + p.category.separability.diff.lowermid + facet_wrap(~height)) +
  plot_annotation(tag_levels = list(c('A', 'B', '', ''))) +
  plot_layout(guides = "collect") &
  theme(legend.position = "right",
        legend.title = element_blank(),
...
Figure \@ref(fig:cat-sep-long-short) indicates that category separability is generally higher for some pairs. For instance, the [`r linguisticsdown::cond_cmpl("eː")`] - [`r linguisticsdown::cond_cmpl("ɛ")`], [`r linguisticsdown::cond_cmpl("ʉː")`] - [`r linguisticsdown::cond_cmpl("ɵ")`], [`r linguisticsdown::cond_cmpl("oː")`] - [`r linguisticsdown::cond_cmpl("ɔ")`], and [`r linguisticsdown::cond_cmpl("ɑː")`] - [`r linguisticsdown::cond_cmpl("a")`] pairs display the largest separability even when only F1-F2 is considered. The inclusion of additional cues unsurprisingly increases category separability for all pairs, more so for pairs that are less separable in F1-F2 space (e.g., [`r linguisticsdown::cond_cmpl("uː")`] - [`r linguisticsdown::cond_cmpl("ʊ")`], [`r linguisticsdown::cond_cmpl("yː")`] - [`r linguisticsdown::cond_cmpl("ʏ")`], [`r linguisticsdown::cond_cmpl("æː")`] - [`r linguisticsdown::cond_cmpl("æ")`], [`r linguisticsdown::cond_cmpl("øː")`] - [`r linguisticsdown::cond_cmpl("ø")`], and [`r linguisticsdown::cond_cmpl("œː")`] - [`r linguisticsdown::cond_cmpl("œ")`]). The inclusion of duration seems to maximize the increase in separability relative to baseline (Figure \@ref(fig:cat-sep-long-short)column B). Interestingly, this is not the case for [`r linguisticsdown::cond_cmpl("yː")`] - [`r linguisticsdown::cond_cmpl("ʏ")`] that achieves the highest separability for the F1-F2-F3 combination. This would overall seem to suggest that the first two formants and duration are the most important cues to long-short vowel pair distinctions, while the inclusion of F3 unsurprisingly does not punish the separability.

These findings were confirmed by the statistical analysis (Summary tables in SI \@ref(sec:SI-LMMs-study1)): the LMMs indicated that the F1-F2-Duration cue combination generated the highest separability for all pairs but the [`r linguisticsdown::cond_cmpl("yː")`] - [`r linguisticsdown::cond_cmpl("ʏ")`] (all other $ps > .0001$). For the [`r linguisticsdown::cond_cmpl("yː")`] - [`r linguisticsdown::cond_cmpl("ʏ")`] contrast, the F1-F2-F3 combination achieved the highest separability ($\hat\beta = .057, SE = .004, p < .0001$). The inclusion of F3 nevertheless increased separability relative to F1-F2 for all pairs (all $ps > .0001$), with the [`r linguisticsdown::cond_cmpl("ɛː")`] - [`r linguisticsdown::cond_cmpl("ɛ")`] ($\hat\beta = .01, SE = .004, p > .015$), [`r linguisticsdown::cond_cmpl("øː")`] - [`r linguisticsdown::cond_cmpl("ø")`] ($\hat\beta = .01, SE = .004, p < .011$) and [`r linguisticsdown::cond_cmpl("œː")`] - [`r linguisticsdown::cond_cmpl("œ")`] ($\hat\beta = .009, SE = .0036, p < .012$) pairs displaying overall smaller albeit statistically significant differences. Additional analyses in the SI suggested that ...<!--add here-->
...
#ggsave(file = "../../figures/p.png", width = 9, height = 5, dpi=1200)
```
(ref:m-high-front-I-U) Fitted smooths of GAMM for predicting F1 (**upper row**), F2 (**mid row**), F3 (**bottom row**) and 95% confidence intervals for the long vowels [`r linguisticsdown::cond_cmpl("iː")`] - [`r linguisticsdown::cond_cmpl("ʉː")`] (**left**), and [`r linguisticsdown::cond_cmpl("iː")`] - [`r linguisticsdown::cond_cmpl("eː")`] (**right**). Differences significantly different from 0 are marked by black dotted vertical lines.

```{r m-high-front-I-U, message=FALSE, fig.width=base.width*4.5, fig.height=base.height*4.5, fig.align='center', fig.cap="(ref:m-high-front-I-U)"}
par(mfrow=c(3,2))
...
(ref:m-high-back) Fitted smooths of GAMM for predicting F1 (**upper row**), F2 (**mid row**), F3 (**bottom row**) and 95% confidence intervals for the long vowels [`r linguisticsdown::cond_cmpl("oː")`] - [`r linguisticsdown::cond_cmpl("uː")`] (**left**), and the short vowels [`r linguisticsdown::cond_cmpl("ɔ")`] - [`r linguisticsdown::cond_cmpl("ʊ")`] (**right**). Differences significantly different from 0 are marked by black dotted vertical lines.
...
(ref:m-center) Fitted smooths of GAMM for predicting F1 (**upper row**), F2 (**mid row**), F3 (**bottom row**) and 95% confidence intervals for the long vowels [`r linguisticsdown::cond_cmpl("ɛː")`] - [`r linguisticsdown::cond_cmpl("æː")`] (**left**), and the short vowels [`r linguisticsdown::cond_cmpl("ɛ")`] - [`r linguisticsdown::cond_cmpl("æ")`] (**right**). Differences significantly different from 0 are marked by black dotted vertical lines.
...
(ref:m-mid) Fitted smooths of GAMM for predicting F1 (**upper row**), F2 (**mid row**), F3 (**bottom row**) and 95% confidence intervals for the long vowels [`r linguisticsdown::cond_cmpl("øː")`] - [`r linguisticsdown::cond_cmpl("œː")`] (**left**), and the short vowels [`r linguisticsdown::cond_cmpl("ø")`] - [`r linguisticsdown::cond_cmpl("œ")`] (**right**). Differences significantly different from 0 are marked by black dotted vertical lines.
