...
m.F1.long <- mgcv::bam(F1 ~ Gender + Quantity * vowel_pair + s(Location, by = category, k = 5) 
                    + s(Talker, category, bs = "re")
                    # + s(Location, Talker,  by = category, bs = "fs", k = 5, m=1)
                    , data = d.model %>% filter(Quantity == "long"))
```
