...
(ref:swe-trad-vowels) The Central Swedish vowel space as represented by the talkers in @kuronen2000. Vowel labels are placed at the vowel mean across talkers. Long vowels are boldfaced.

```{r swe-trad-vowels, fig.width=base.width*3, fig.height=base.height*3, out.width='60%', fig.align='center', fig.cap="(ref:swe-trad-vowels)"}
read_delim('../../data/phonetic vowel statistics/Swedish/Published databases/Kuronen2000/kuronen_L1_male_talker_formants.csv') %>%
  mutate(
...
Study 1 describes the spectral acoustics of Central Swedish in static and dynamic analyses of F0, F1, F2, F3, and duration. While fundamental frequency (F0) is not considered an important cue to vowel identity in itself, it is known to vary between languages, dialects and speech styles [e.g., @henton2005; @jacewicz-fox2018; @johnson2005; @mennen2012; @weirich2019] and is therefore also reported. Measurements of vowel duration are included to capture the systematic quantity distinction between long-short vowel pairs in Central Swedish. Besides providing a detailed description of the acoustics of Central Swedish vowels, Study 1 also aims for an evaluation of the relative importance of certain cues for some vowel distinctions as hypothesized in previous work, such as the importance of lip-rounding for high vowel distinctions and to what extent long-short vowel pairs differ in quality (spectral cues) and quantity [@refs], as well as evaluating which cues carry information about vowel quality once dynamic information is considered.
...
```{r swe-vowels, fig.width=base.width*5, fig.height=base.height*4, out.width='90%', fig.align='center', fig.pos="h", fig.cap="(ref:swe-vowels)"}
...
In the present study, GAMMs were used for assessing the formant dynamics across all 21 categories, as well as contrasting long and short vowels in each pair against each other, in order to...<!--add here--> 
...
get_category_separability <- function(
  ...
  data %>%
    ...
    mutate(
      ...
      overall_separability = (sum(sqrt(category_separability_F1 + category_separability_F2))))
}
...
d.SwehVd.for.cat.separability.all <- rbind(
  ...
  group_by(category) %>%
  summarise(
    category_separability = mean(overall_separability, na.rm = T))
```

#### Quantity vs. quality in long and short vowel pairs
To gain more insight into the extent to which there are spectral and temporal differences between long and short vowels, the acoustics of categories within vowel pairs are evaluated. Next, we ask whether quantity and quality distinctions seem to be separate from each other.
...
```
```{r}
p.trajectories.ellipse <- d.SwehVd.forStudy.long.dynamic %>%
  # Filter down to test folds
  filter(Normalization.Type == "Nearey's uniform scaling (log)", timepoint %in% c(20, 65, 80), category %in% c("[eː]", "[ɛː]", "[æː]", "[øː]", "[ø]", "[œː]", "[œ]", "[oː]", "[uː]")) %>%
  mutate(timepoint = as.character(timepoint)) %>%
  ...
  geom_label(
    data =
      d.SwehVd.forStudy.long.dynamic %>%
      filter(Normalization.Type == "Nearey's uniform scaling (log)", timepoint == 50, category %in% c("[eː]", "[ɛː]", "[æː]", "[øː]", "[ø]", "[œː]", "[œ]", "[oː]", "[uː]")) %>%
      mutate(timepoint = as.character(timepoint))  %>%
      group_by(Normalization.Type, Talker, category, timepoint, Quantity) %>%
      summarise(across(c(F1, F2), ~mean(.x))) %>%
...
\@ref(fig:SI-m-F3) in SI visualizes all categories included in the GAMMs for F3).^[A main effect of Gender was only found for F2 and the short vowels ($\hat\beta = -.028, se = .01, p > .006$).]

The separate GAMMs fitted to F1, F2 and F3 for the long and short vowels suggest that when formant dynamics is considered, the overlap in F1 and F2 for the [`r linguisticsdown::cond_cmpl("iː")`] - [`r linguisticsdown::cond_cmpl("yː")`] distinction is likely reduced. Given the absence of main effect for F3, it might be reasonable to assume that the dynamics in F1 and F2 is presumably sufficient for a reliable distinction between the two categories...<!--add here-->
...
### Results summary
The static and dynamic analyses suggest that F1, F2, F3 and duration are important for vowel identity in Central Swedish. It is reasonable to assume formant dynamics as auxiliary cue to vowel identity, more so for some distinctions than others, adding to the relative contribution of each cue, as indicated by...<!--add here--> For instance, the [`r linguisticsdown::cond_cmpl("iː")`] - [`r linguisticsdown::cond_cmpl("yː")`], [`r linguisticsdown::cond_cmpl("oː")`] - [`r linguisticsdown::cond_cmpl("uː")`] and [`r linguisticsdown::cond_cmpl("ɛː")`] - [`r linguisticsdown::cond_cmpl("æː")`] categories display substantial overlap in static analyses but increased separability when analysed dynamically. The dynamic analyses furthermore highlighted that the short vowels also display formant movements, and that for most of both long and short categories, a larger portion of the dynamics resides in the later part of the segment. The results furthermore suggested that F3 does not seem to distinguish between [`r linguisticsdown::cond_cmpl("iː")`] - [`r linguisticsdown::cond_cmpl("yː")`]...

One way to assess the relative contribution of each cue to the entire space is to fit a brms...???
<!--A relatively indirect estimate of how much each cue contributed to vowel distinctions in the entire 21-category space was obtained by comparing the proportion of explained deviance in the GAMMs predicting F1, F2, F3 and duration from all vowel categories. These models confirmed the general pattern, suggesting that F1 (93.7% of explained deviance), F2 (96.2% of explained deviance) and duration (92% of explained deviance) can more easily predict all 21 categories than F3 (68.5% of explained deviance).-->
