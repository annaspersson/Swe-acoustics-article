...
get_category_separability <- function(
  ...
  data %>% 
    ...
    mutate(
      ...
      category_separabilityF0F1F2F3duration = sum(sqrt((F0 - F0_mean)^2 + (F1 - F1_mean)^2 + (F2 - F2_mean)^2 + (F3 - F3_mean)^2 + (Duration - Duration_mean)^2)) / n())
}
...
d.SwehVd.cat.separability <- rbind(
  ...
  summarise(
    ...
    F0F1F2 = mean(category_separabilityF0F1F2),
    F0F1F2F3duration = mean(category_separabilityF0F1F2F3duration)) %>%
  pivot_longer(
    cols = starts_with("F"),
...
In order to evaluate what cue combination would provide the largest separability between vowels in long-short contrasts, the category separability index was calculated for each pair and three different cue combinations: F1-F2, F1-F2-F3, or F0-F1-F2.^[For an approximation of the relative separability by cue, evaluations of F1-F3, F2-F3, F0-F1 and F0-F2 are included in the SI \@ref(sec:SI-XXX).]<!--add here-->
